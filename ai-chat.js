...
const storedModel = localStorage.getItem('welkin_ai_model') || 'meta/llama-3.2-11b-vision-instruct';
...
if (!storedKey) {
    setTimeout(() => {
        if (!localStorage.getItem('welkin_ai_api_key')) {
            addMessage('ai', 'Welcome! To start chatting, please add your NVIDIA or Gemini API key in the settings.');
        }
    }, 1000);
}
...
function saveSettings() {
    ...
    if (key) {
        ...
        toggleSettings();
        addMessage('ai', `Settings saved! Provider set to ${provider === 'nvidia' ? 'NVIDIA NIM' : 'Google Gemini'}.`);
    } else {
...
function addMessage(role, content, image = null) {
    ...
    // Add to history
    chatHistory.push({ role: role === 'user' ? 'user' : 'model', parts: [{ text: content }] });
}
...
async function callAI(prompt, imageBase64 = null) {
    ...
    if (!key) {
        addMessage('ai', 'Error: API Key missing. Please go to Settings and add your API Key.');
        return;
    }
...
async function handleSend() {
    ...
    if (text) {
        addMessage('user', text, imageToSend === currentImageBase64 ? null : imageToSend);
    } else if (imageToSend) {
