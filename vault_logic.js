...
function generateDescription(name) {
  const descriptions = {
    Minecraft:
      "Build, explore, and survive in the ultimate block-based sandbox.",
    ...
    "Five Nights at Freddy's":
      "Can you survive five nights at the world's scariest pizzeria?",
...
function initVault() {
  ...
  const popularGames = allGames
    .filter((g) => popularSelection.some((p) => g.name.includes(p)))
    .slice(0, 5);
  heroGames = allGames.filter((g) =>
    popularSelection.some((p) => g.name.includes(p)),
  );

  if (popularList) {
    popularList.innerHTML = popularGames
      .map((game) => {
...
function renderGames(gamesToRender) {
  ...
  vaultGrid.innerHTML = gamesToRender
    .map((game) => {
      const escapedName = game.name.replace(/'/g, "\\'");
      const escapedFile = game.file.replace(/'/g, "\\'");
