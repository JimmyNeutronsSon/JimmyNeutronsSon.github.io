...
(function () {
  ...
  const buildAudio = (container) => {
    ...
    const playSong = (song) => {
      ...
      artistEl.innerText = Array.isArray(song.primaryArtists)
        ? song.primaryArtists.map((a) => a.name).join(", ")
        : typeof song.primaryArtists === "object"
          ? song.primaryArtists.name
    ...
    const search = async (q) => {
      ...
      try {
        ...
        const [songsRes, albumsRes] = await Promise.all([
          ...
          fetchApi(
            `${base}/search/albums?query=${encodeURIComponent(q)}&limit=5`,
          ),
        ]);
        ...
        if (songsRes.data?.results?.length) {
          results.innerHTML +=
            '<div style="font-size:12px; color:#3b82f6; font-weight:bold; margin-bottom:10px; text-transform:uppercase;">Songs</div>';
          songsRes.data.results.forEach((s, i) => {
            const div = document.createElement("div");
            div.style.cssText =
              "display:flex; align-items:center; gap:12px; padding:10px; cursor:pointer; border-radius:10px; margin-bottom:8px; background:rgba(255,255,255,0.03);";
        ...
        if (albumsRes.data?.results?.length) {
          results.innerHTML +=
            '<div style="font-size:12px; color:#3b82f6; font-weight:bold; margin:15px 0 10px; text-transform:uppercase;">Albums</div>';
          albumsRes.data.results.forEach((al) => {
            ...
            div.onclick = async () => {
              results.innerHTML =
                '<div style="color:#aaa;text-align:center;padding:20px;">Loading album...</div>';
              const alData = await fetchApi(`${base}/albums?id=${al.id}`);
              queue = alData.data.songs;
              curIdx = 0;
              playSong(queue[0]);
              search(q);
            };
    ...
    container.querySelector("#m-next").onclick = () => {
      if (queue.length > curIdx + 1) playSong(queue[++curIdx]);
    };
    container.querySelector("#m-prev").onclick = () => {
      if (curIdx > 0) playSong(queue[--curIdx]);
    };
  };
