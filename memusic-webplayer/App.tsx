...
const ApiPlaylistView = lazy(
  () => import("./components/views/ApiPlaylistView"),
);
...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="h-screen w-screen bg-[#0B1E3D] text-white flex flex-col items-center justify-center p-8 text-center font-sans glass-panel">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              ...
              className="px-6 py-3 rounded-full bg-[#1E6CC7] text-white font-bold hover:bg-[#3A8FE0] transition-colors shadow-lg shadow-blue-500/20"
            >
              Reset App & Reload
            </button>
          </div>
  ...
  return (
    ...
    >
      <ProfileProvider profile={appState.profile} setAppState={setAppState}>
        <UserMusicProvider
          musicData={appState.music}
          setAppState={setAppState}
          fullState={appState}
        >
          <PlayerProvider
            playerSettings={appState.settings.player}
            playerQueue={appState.playerQueue}
            setAppState={setAppState}
          >
            <PartyProvider>
              <ErrorBoundary>
...
const MainApp: React.FC<MainAppProps> = ({ searchHistory, setAppState }) => {
  ...
  const { partyState, partyEndedMessage, clearPartyEndedMessage } =
  ...
  const navigate = useCallback(
    (entry: Omit<HistoryEntry, "key">, replace = false) => {
  ...
  const navigateToApiPlaylist = (playlist: Playlist) =>
    navigate({ view: "api_playlist", apiPlaylist: playlist });
  const navigateToArtist = (artistId: string) =>
    navigate({ view: "artist", artistId });
  ...
  const renderView = () => {
    switch (currentViewEntry.view) {
      case "home":
        return (
          <Home
            ...
            navigateToApiPlaylist={navigateToApiPlaylist}
      ...
      case "search":
        return (
          <Search
            ...
            navigateToArtist={navigateToArtist}
            navigateToApiPlaylist={navigateToApiPlaylist}
            initialQuery={currentViewEntry.searchQuery}
          />
        );
      case "library":
        return (
      ...
      case "album":
        return (
          <AlbumView
            albumId={currentViewEntry.albumId!}
            setActiveView={changeView}
            navigateToArtist={navigateToArtist}
            navigateToPlaylist={navigateToPlaylist}
          />
      ...
      case "playlist":
        return (
          <PlaylistView
            playlistId={currentViewEntry.playlistId!}
            setActiveView={changeView}
            navigateToArtist={navigateToArtist}
          />
        );
      case "api_playlist":
        return (
          <ApiPlaylistView
            playlist={currentViewEntry.apiPlaylist!}
            setActiveView={changeView}
            navigateToArtist={navigateToArtist}
          />
        );
      case "artist":
        return (
          <ArtistView
            artistId={currentViewEntry.artistId!}
            setActiveView={changeView}
            navigateToAlbum={navigateToAlbum}
            navigateToArtist={navigateToArtist}
          />
  ...
  const highQualityImage =
    displayedSong?.image?.find((img) => img.quality === "500x500")?.url ||
    displayedSong?.image?.[0]?.url ||
    "";
  const partyEndedMessageText = partyEndedMessage
    ? t(partyEndedMessage.key, partyEndedMessage.replacements)
    : null;

  return (
    <ModalContext.Provider value={{ showModal, hideModal }}>
      <div className="relative h-screen w-screen overflow-hidden text-gray-200 font-sans selection:bg-[#3A8FE0] selection:text-white">
        <CinematicBackground
          songImage={highQualityImage}
          isPlaying={isPlaying}
        />
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="flex flex-1 overflow-hidden relative">
            ...
            <div
              className={`flex-shrink-0 overflow-hidden transition-[width] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] border-l border-white/5 glass-panel ${isQueueOpen ? "w-96" : "w-0"}`}
            >
          ...
          <div
            className={`z-50 shrink-0 transition-[height] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${displayedSong ? "h-24 md:h-28" : "h-0"}`}
          >
            <div
              className={`h-full transition-opacity duration-500 ${displayedSong ? "opacity-100" : "opacity-0"}`}
            >
        ...
        >
          ...
          <div className="flex justify-end">
            <button
              onClick={clearPartyEndedMessage}
              className="px-6 py-2 rounded-full bg-[#1E6CC7] text-white font-bold hover:bg-[#3A8FE0] transition-colors shadow-lg shadow-blue-500/20"
            >
              {t("modals.partyEnded.ok")}
            </button>
          </div>
        </Modal>
      </div>
