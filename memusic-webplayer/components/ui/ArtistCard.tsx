...
export const ArtistCard: React.FC<ArtistCardProps> = React.memo(({ artist, onArtistClick }) => {
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-square mb-4 overflow-hidden rounded-full shadow-lg border-2 border-transparent group-hover:border-[#3A8FE0]/50 transition-colors duration-300">
        <img 
            src={imageUrl} 
            alt={artist.name} 
      ...
      <h4 className="font-bold text-white truncate text-lg tracking-tight group-hover:text-[#3A8FE0] transition-colors">{artist.name}</h4>
