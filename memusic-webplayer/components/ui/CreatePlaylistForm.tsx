...
export const CreatePlaylistForm: React.FC<CreatePlaylistFormProps> = ({
  ...
  const [description, setDescription] = useState(
    initialSong
      ? `Starts with "${initialSong.name}" by ${initialSong.artists.primary.map((a) => a.name).join(", ")}`
      : "",
  );
    ...
    <>
      <div className="text-gray-300 mb-6 mt-4 space-y-4">
        <input
          ...
          className="w-full bg-white/10 p-3 rounded-md focus:outline-none focus:ring-2 focus:ring-[#3A8FE0]"
      ...
      <div className="flex justify-end space-x-4">
        <button
          onClick={onCancel}
          className="px-5 py-2.5 rounded-md bg-white/10 hover:bg-white/20 font-semibold transition-colors"
        >
          {t("modals.createPlaylist.cancel")}
        </button>
        <button
          onClick={handleConfirm}
          disabled={!isNameValid}
          className="px-5 py-2.5 rounded-md bg-[#3A8FE0] text-white font-bold disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
        >
