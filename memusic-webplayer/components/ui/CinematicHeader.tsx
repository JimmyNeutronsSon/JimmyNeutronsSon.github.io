...
export const CinematicHeader: React.FC<CinematicHeaderProps> = ({
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <div
        className={`fixed top-0 left-0 right-0 z-40 md:pl-64 h-20 bg-[#0B1E3D]/90 backdrop-blur-2xl border-b border-white/5 flex items-center justify-between px-6 transition-all duration-500 transform ${isScrolled ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}
      >
        <div className="flex items-center gap-4 overflow-hidden">
          <button
            onClick={onPlay}
            className="w-12 h-12 bg-[#3A8FE0] rounded-full flex items-center justify-center text-black shadow-[0_0_20px_rgba(58,143,224,0.4)] hover:scale-105 hover:bg-[#6CB4F0] active:scale-95 transition-all duration-300 flex-shrink-0"
          >
          ...
          <span className="font-bold text-xl text-white truncate tracking-tight">
            {title}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`p-2.5 rounded-full bg-white/5 hover:bg-white/10 transition-all active:scale-95 ${isFavorite ? "text-[#3A8FE0]" : "text-gray-400"}`}
          >
            <HeartIcon
      ...
      <div className="relative w-full overflow-hidden group">
        <div className="absolute inset-0 z-0 h-[85vh] select-none pointer-events-none overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-[#0B1E3D]/40 to-[#0B1E3D]" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0B1E3D] via-[#0B1E3D]/80 to-transparent" />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-transparent via-[#0B1E3D]/20 to-[#0B1E3D]" />
        </div>
            ...
            <div className="w-52 h-52 md:w-72 md:h-72 relative overflow-hidden rounded-2xl bg-white/5 border border-white/10 shadow-2xl ring-1 ring-white/5 z-10 transform transition-transform duration-500 group-hover/image:scale-[1.02]">
              {imageUrl ? (
                <img
                  src={imageUrl}
                  alt={title}
                  className="w-full h-full object-cover shadow-inner animate-image-appear"
                />
              ) : (
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4 pb-2">
            <div className="flex items-center gap-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/5 backdrop-blur-md text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-white shadow-sm">
                {isVerified && (
                  <VerifiedIcon className="w-3.5 h-3.5 text-blue-400 -ml-0.5" />
                )}
                {type}
              </span>
            </div>
            ...
            <div className="group/title relative">
              <h1
                className={`text-5xl md:text-7xl lg:text-8xl font-black text-white tracking-tighter leading-[0.9] drop-shadow-2xl mix-blend-overlay opacity-90 ${isEditable ? "cursor-pointer hover:underline decoration-[#3A8FE0] decoration-4 underline-offset-8" : ""}`}
                onClick={() => isEditable && onEditTitle?.()}
              >
                {title}
                {isEditable && (
                  <PencilIcon className="w-8 h-8 inline-block ml-4 text-gray-500 opacity-0 group-hover/title:opacity-100 transition-opacity" />
                )}
              </h1>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm font-semibold text-gray-300 mt-4 tracking-wide">
              {subtitle && (
                <span className="text-white border-r border-white/20 pr-4">
                  {subtitle}
                </span>
              )}
              {meta && <span>{meta}</span>}
        ...
        <div className="relative z-10 px-6 md:px-12 pb-6">
          <div className="inline-flex items-center p-2 pr-6 rounded-[2rem] bg-white/5 border border-white/5 backdrop-blur-xl shadow-xl hover:bg-white/10 transition-colors duration-300 gap-4">
            <button
              onClick={onPlay}
              className="relative w-16 h-16 bg-[#3A8FE0] text-white rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(58,143,224,0.5)] hover:scale-105 hover:bg-[#6CB4F0] active:scale-95 transition-all duration-300 group"
            >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title={
                  isFavorite
                    ? t("albumView.removeFromFav")
                    : t("albumView.addToFav")
                }
              >
                <HeartIcon
                  className={`w-7 h-7 transition-all duration-300 ${isFavorite ? "fill-[#3A8FE0] text-[#3A8FE0] drop-shadow-[0_0_10px_rgba(58,143,224,0.5)]" : "text-gray-300 group-hover/fav:text-white"}`}
                />
              </button>
