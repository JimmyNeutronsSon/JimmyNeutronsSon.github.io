...
const SongListItem: React.FC<SongListItemProps> = React.memo(({ song, index, songs, playlistId, context, navigateToArtist, onInteraction, navigateToPlaylist }) => {
    ...
    return (
        <div onMouseLeave={() => setActiveMenu(false)} className={`group flex items-center p-3 rounded-xl transition-all duration-200 border border-transparent ${isPlayingCurrent ? 'bg-white/10 border-white/5' : 'hover:bg-white/5 hover:border-white/5'} cursor-pointer`}>
            <div className={`w-8 text-center mr-4 flex-shrink-0 font-medium ${isPlayingCurrent ? 'text-[#3A8FE0]' : 'text-gray-500 group-hover:hidden'}`}>
                {isPlayingCurrent ? (
                    isPlaying ? <PauseIcon className="w-4 h-4 mx-auto text-[#3A8FE0]" /> : <PlayIcon className="w-4 h-4 mx-auto text-[#3A8FE0]" />
                ) : index + 1}
            </div>
            <button onClick={handlePlay} className={`w-8 h-8 items-center justify-center rounded-full text-white mr-4 flex-shrink-0 bg-[#3A8FE0] shadow-lg shadow-[#3A8FE0]/30 ${isPlayingCurrent ? 'hidden' : 'hidden group-hover:flex animate-in zoom-in'}`}><PlayIcon className="w-4 h-4 ml-0.5" /></button>
            
            <img src={imageUrl} alt={song.name} className={`w-12 h-12 rounded-lg mr-4 flex-shrink-0 object-cover shadow-sm transition-transform duration-300 ${isPlayingCurrent ? 'scale-105 shadow-[#3A8FE0]/20' : 'group-hover:scale-105'}`} loading="lazy" />
            
            <div className="flex-1 min-w-0" onClick={handlePlay}>
                <p className={`font-semibold leading-snug line-clamp-1 ${isPlayingCurrent ? 'text-[#3A8FE0]' : 'text-white'}`} title={song.name}>{song.name}</p>
            ...
            <div className="flex items-center space-x-2 ml-4 relative">
                <button onClick={(e) => { e.stopPropagation(); handleMenuAction(() => toggleFavoriteSong(song)); }} className="text-gray-400 hover:text-[#3A8FE0] p-2 rounded-full hover:bg-white/10 transition-colors"><HeartIcon className={`w-5 h-5 transition-transform active:scale-125 ${isFavoriteSong(song.id) ? 'fill-[#3A8FE0] text-[#3A8FE0] drop-shadow-[0_0_5px_rgba(58,143,224,0.5)]' : ''}`} /></button>
