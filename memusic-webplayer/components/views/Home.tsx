...
import { getArtistDetails, getSongSuggestions, getAlbumDetails } from '../../services/jioSaavnApi';
...
const KANYE_ALBUM_IDS = ['1126967', '1117886', '1945440', '13434529', '3002873'];
...
const HeroCarousel: React.FC<{
    ...
    isMini?: boolean;
}> = ({ albums, activeIndex, onPlay, onClick, isMini }) => {
    if (albums.length === 0) return null;

    return (
        <div className={`relative w-full ${isMini ? 'h-[45vh]' : 'h-[60vh] md:h-[70vh]'} flex items-center justify-center overflow-hidden py-10 px-4`}>
            <div className="relative w-full max-w-7xl h-full flex items-center justify-center">
                {albums.slice(0, 5).map((album, index) => {
                    ...
                    const size = isMini ? (isActive ? 'w-[200px] h-[200px]' : 'w-[150px] h-[150px]') : (isActive ? 'w-[320px] h-[320px] md:w-[450px] md:h-[450px]' : 'w-[240px] h-[240px] md:w-[350px] md:h-[350px]');

                    return (
                        <div
                            key={album.id}
                            onClick={() => isActive ? onClick(album.id) : null}
                            className={`absolute transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] cursor-pointer
                                ${isActive ? 'z-30 opacity-100' : 'z-10 opacity-40 hover:opacity-60'} ${size}
                            `}
                            style={{
                                transform: `translateX(${position * (isMini ? 50 : 60)}%) scale(${isActive ? 1 : 0.8}) rotateY(${position * -20}deg)`,
                                filter: isActive ? 'none' : 'blur(2px) grayscale(0.5)',
                            }}
                        >
                            <div className="relative w-full h-full rounded-[2.5rem] overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.5)] border border-white/10 group">
                                <img
                                    src={imageUrl}
                                    alt={album.name}
                                ...
                                <div className={`absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
                                    <div className="absolute inset-0 p-8 flex flex-col justify-end">
                                        <h2 className={`font-black text-white leading-tight mb-1 drop-shadow-lg line-clamp-2 ${isMini ? 'text-lg' : 'text-2xl md:text-4xl'}`}>
                                            {album.name}
                                        </h2>
                                        <p className={`text-white/70 font-medium ${isMini ? 'text-xs mb-2' : 'text-sm md:text-lg mb-4'}`}>
                                            {album.artists.primary[0]?.name}
                                        </p>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); onPlay(album.id); }}
                                            className={`self-start flex items-center justify-center bg-white/20 hover:bg-white/30 backdrop-blur-xl rounded-full border border-white/20 transition-all active:scale-95 group/play ${isMini ? 'w-10 h-10' : 'w-12 h-12 md:w-16 md:h-16'}`}
                                        >
                                            <PlayIcon className={`${isMini ? 'w-4 h-4' : 'w-6 h-6 md:w-8 md:h-8'} text-white translate-x-0.5 group-hover/play:scale-110 transition-transform`} />
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-40">
                {albums.slice(0, 5).map((_, i) => (
                    <div
                        key={i}
                        className={`h-1.5 rounded-full transition-all duration-300 ${i === activeIndex ? 'w-8 bg-[#3A8FE0]' : 'w-1.5 bg-white/20'}`}
                    />
                ))}
            </div>
        </div>
...
const HeroSection: React.FC<{ album: Album; onPlay: () => void; onClick: () => void; }> = ({ album, onPlay, onClick }) => {
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#0B1E3D] via-[#0B1E3D]/40 to-transparent opacity-90"></div>
            <div className="absolute inset-0 bg-gradient-to-r from-[#0B1E3D]/80 via-transparent to-transparent"></div>
            ...
            <div className="absolute inset-0 p-8 md:p-16 flex flex-col justify-end items-start">
                <div className="transform transition-all duration-700 translate-y-4 group-hover:translate-y-0 opacity-0 animate-in fade-in slide-in-from-bottom-8 fill-mode-forwards" style={{ animationDelay: '0.1s' }}>
                    <span className="inline-flex items-center gap-2 px-4 py-1.5 mb-6 text-xs font-bold tracking-[0.2em] text-white uppercase bg-white/10 backdrop-blur-md rounded-full border border-white/10 shadow-lg">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#3A8FE0] animate-pulse"></span>
                        Featured Release
                    </span>
                    ...
                    <button
                        ...
                        className="group/btn relative flex items-center gap-4 px-8 py-4 bg-[#3A8FE0] text-white rounded-full overflow-hidden transition-all duration-300 hover:pr-6 hover:shadow-[0_0_40px_rgba(58,143,224,0.4)]"
                    >
    ...
    return (
        ...
        >
            <div className="relative w-16 h-16 md:w-20 md:h-20 flex-shrink-0 rounded-xl overflow-hidden shadow-lg">
                ...
                >
                    <div className="w-10 h-10 bg-[#3A8FE0] rounded-full flex items-center justify-center text-black shadow-lg transform scale-50 group-hover:scale-100 transition-transform duration-300">
            ...
            <div className="ml-4 flex-1 min-w-0 pr-2">
                <p className="font-bold text-white truncate text-base group-hover:text-[#3A8FE0] transition-colors">{item.name}</p>
    ...
    return (
        ...
        >
            <div className="relative w-full aspect-square mb-4 overflow-hidden rounded-2xl shadow-lg border border-white/5">
                ...
                <button
                    onClick={handlePlay}
                    className="absolute bottom-4 right-4 w-12 h-12 bg-[#3A8FE0] rounded-full flex items-center justify-center text-black shadow-xl shadow-black/30 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 ease-out"
            ...
            <div className="px-1">
                <h4 className="font-bold text-white truncate text-base leading-tight group-hover:text-[#3A8FE0] transition-colors">{song.name}</h4>
    ...
    return (
        <section className="relative">
            ...
            <div className="relative group">
                {/* Fade Masks */}
                <div className={`absolute top-0 bottom-0 left-0 w-24 bg-gradient-to-r from-[#0B1E3D] to-transparent z-10 pointer-events-none transition-opacity duration-500 ${showLeftFade ? 'opacity-100' : 'opacity-0'}`} />
                <div className={`absolute top-0 bottom-0 right-0 w-24 bg-gradient-to-l from-[#0B1E3D] to-transparent z-10 pointer-events-none transition-opacity duration-500 ${showRightFade ? 'opacity-100' : 'opacity-0'}`} />

                {/* Navigation Buttons */}
                <button
                    onClick={() => scroll('left')}
                    className={`absolute left-2 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 backdrop-blur-md border border-white/10 text-white flex items-center justify-center z-20 transition-all duration-300 hover:bg-[#3A8FE0] hover:scale-110 shadow-xl ${showLeftFade ? 'opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0' : 'opacity-0 pointer-events-none'}`}
...
const Home: React.FC<HomeProps> = ({ setActiveView, navigateToAlbum, navigateToArtist, navigateToSearch, navigateToApiPlaylist, navigateToPlaylist }) => {
    ...
    const [hipHopSpotlight, setHipHopSpotlight] = useState<{ artists: any[], albums: Album[], songs: Song[] }>({ artists: [], albums: [], songs: [] });
    ...
    useEffect(() => {
        const fetchSpotlight = async () => {
            ...
            try {
                // Kanye West, Drake, Kendrick Lamar, Travis Scott
                const HIP_HOP_IDS = ['527097', '512453', '612060', '614647'];
                const [artistResponses] = await Promise.all([
                    Promise.all(HIP_HOP_IDS.map(id => getArtistDetails(id)))
                ]);
                ...
                artistResponses.forEach(res => {
                    if (res.success) {
                        ...
                        if (res.data.topAlbums) allAlbums.push(...res.data.topAlbums);
                        if (res.data.topSongs) allSongs.push(...res.data.topSongs);
                    }
                });


                // Fetch specific albums to be sure
                const specificAlbums = await Promise.all(KANYE_ALBUM_IDS.map(id => getAlbumDetails(id)));
                const validSpecific = specificAlbums.filter(res => res.success).map(res => res.data);

                // Filter out specific albums from the general list to avoid duplicates
                const otherAlbums = allAlbums.filter(a => !KANYE_ALBUM_IDS.includes(a.id));

                // Shuffle albums and songs
                const shuffledAlbums = [...validSpecific, ...otherAlbums.sort(() => 0.5 - Math.random())].slice(0, 20);
                const shuffledSongs = allSongs.sort(() => 0.5 - Math.random()).slice(0, 15);
    ...
    const isLoading = loading.releases || loading.recommendations || loading.spotlight;

    const heroAlbums = useMemo(() => {
        const kanyes = hipHopSpotlight.albums.filter(a => KANYE_ALBUM_IDS.includes(a.id));
        const others = newReleases.length > 0 ? newReleases.filter(a => !KANYE_ALBUM_IDS.includes(a.id)) : hipHopSpotlight.albums.filter(a => !KANYE_ALBUM_IDS.includes(a.id));
        // Ensure kanyes are always first
        return [...kanyes, ...others].slice(0, 10);
    }, [newReleases, hipHopSpotlight.albums]);

    useEffect(() => {
        if (heroAlbums.length <= 1) return;
        const interval = setInterval(() => {
            setHeroIndex(prev => (prev + 1) % Math.min(heroAlbums.length, 5));
        }, 2000); // rotate every 2 seconds
        return () => clearInterval(interval);
    }, [heroAlbums]);

    const heroRelease = heroAlbums[heroIndex] || null;
    const remainingReleases = newReleases.length > 0 ? newReleases.filter(a => a.id !== heroRelease?.id) : [];

    return (
        <div className={`text-white ${isMini ? 'pb-10' : 'pb-32'}`}>
            {isLoading && !heroRelease ? (
                <div className="flex justify-center items-center h-[80vh]"><Loader /></div>
            ) : (
                <div className={isMini ? 'space-y-4' : 'space-y-16'}>
                    <div className={isMini ? 'px-2 pt-2' : 'px-6 md:px-10 pt-8'}>
                        {!isMini && (
                            <div className="flex flex-col mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
                                <p className="text-sm font-bold text-[#3A8FE0] uppercase tracking-widest mb-1 opacity-80">{getDateString()}</p>
                        ...
                        {heroAlbums.length > 0 && (
                            <div className="animate-in fade-in zoom-in-95 duration-1000">
                                <HeroCarousel
                                    ...
                                    onPlay={(id) => navigateToAlbum(id)}
                                    onClick={(id) => navigateToAlbum(id)}
                                    isMini={isMini}
                                />
                            </div>
                        )}
                    ...
                    {!isMini && recommendations.length > 0 && (
                    ...
                    {!isMini && hipHopSpotlight.artists.length > 0 && (
                        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
                            <HorizontalScroller title="Hip-Hop Titans">
                                {hipHopSpotlight.artists.map(artist => (
                                    <div key={artist.id} className="w-36 md:w-48 flex-shrink-0 snap-start">
                                        <ArtistCard artist={artist} onArtistClick={navigateToArtist} />
                                    </div>
                                ))}
                            </HorizontalScroller>
                        </div>
                    ...
                    {hipHopSpotlight.albums.length > 0 && (
                        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
                            <HorizontalScroller title="Legendary Albums">
                                {hipHopSpotlight.albums.map(album => (
                    ...
                    {!isMini && hipHopSpotlight.songs.length > 0 && (
                        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
                            <HorizontalScroller title="Hip-Hop Essentials">
                                {hipHopSpotlight.songs.map(song => (
                                    <div key={song.id} className="snap-start">
                                        <SongCard song={song} />
                                    </div>
                                ))}
                    ...
                    {!isMini && remainingReleases.length > 0 && (
                        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
