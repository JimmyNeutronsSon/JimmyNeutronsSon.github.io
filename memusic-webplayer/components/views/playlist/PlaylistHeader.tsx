...
export const PlaylistHeader: React.FC<PlaylistHeaderProps> = ({
      ...
      content: (
        <>
          <p className="text-gray-300 mb-6">
            {t("playlistView.deletePlaylistConfirm", { name: playlist.name })}
          </p>
        ...
        <>
          <div className="space-y-4 text-gray-300 mb-6 mt-4">
            <input
              ...
              className="w-full bg-white/10 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3A8FE0]"
              placeholder={t("playlistView.playlistName")}
            />
            <textarea
              rows={3}
              ...
              className="w-full bg-white/10 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#3A8FE0]"
              placeholder={t("playlistView.description")}
            />
          </div>
          <div className="flex justify-end space-x-4">
            <button
            ...
            <button
              ...
              className="px-6 py-2 rounded-lg bg-[#3A8FE0] text-white font-bold shadow-lg shadow-[#3A8FE0]/20"
            >
              {t("playlistView.save")}
            </button>
          </div>
  ...
  const handleDownloadM3U = () => {
    ...
    playlist.songs.forEach((song) => {
      ...
      const url =
        song.downloadUrl.find((q) => q.quality === selectedQuality)?.url ||
        song.downloadUrl.find((q) => q.quality === "320kbps")?.url ||
        song.downloadUrl[0]?.url;
      if (url) {
  ...
  const totalDuration =
    playlist.songs?.reduce((acc, song) => acc + (song.duration || 0), 0) || 0;
    ...
    <>
      ...
      >
        <div className="flex flex-col py-1">
          ...
          >
            <DownloadIcon className="w-4 h-4" />
            {t("albumView.downloadM3U")}
          </button>
          <div className="h-px bg-white/10 my-1 mx-2" />
          <button
            onClick={handleDelete}
            className="w-full flex items-center gap-3 text-left px-3 py-2 text-sm rounded-lg text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors"
          >
