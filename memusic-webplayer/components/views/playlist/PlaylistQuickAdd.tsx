...
const AddSongsContent: React.FC<AddSongsContentProps> = ({
  playlistId,
  playlistSongs,
  navigateToArtist,
  onClose,
  ...
  const { previewingSongId, isPreviewPlaying, previewProgress, handlePreview } =
    usePreviewPlayer();
  ...
  const renderSongItem = (song: Song) => {
    const isAdded =
      addedSongs.has(song.id) || playlistSongs.some((ps) => ps.id === song.id);
    const imageUrl =
    ...
    return (
      <div
        key={song.id}
        className="group flex items-center p-3 rounded-2xl hover:bg-white/5 transition-all duration-300 animate-in fade-in slide-in-from-bottom-2 border border-transparent hover:border-white/5"
      >
        <div className="w-14 h-14 rounded-xl mr-4 flex-shrink-0 relative overflow-hidden shadow-lg group-hover:shadow-xl transition-shadow">
          ...
          <button
            ...
            aria-label={
              isPreviewingThisSong && isPreviewPlaying
                ? "Pause preview"
                : "Play preview"
            }
          ...
          >
            {isPreviewingThisSong && isPreviewPlaying ? (
              <PauseIcon className="w-6 h-6 drop-shadow-md" />
            ) : (
              <PlayIcon className="w-6 h-6 drop-shadow-md" />
            )}
          </button>
        </div>

        <div className="flex-1 min-w-0 pr-4">
          <p className="font-bold text-white text-base truncate group-hover:text-[#3A8FE0] transition-colors">
            {song.name}
          </p>
        ...
        >
          <span
            className={`absolute transition-all duration-300 ${isAdded ? "opacity-0 scale-50" : "opacity-100 scale-100"}`}
          >
            Add
          </span>
          <span
            className={`absolute flex items-center gap-1 transition-all duration-300 ${isAdded ? "opacity-100 scale-100" : "opacity-0 scale-150"}`}
          >
  ...
  return (
    <div className="flex flex-col h-full bg-[#0B1E3D]/95 backdrop-blur-3xl">
      <div className="relative p-6 shrink-0 border-b border-white/5">
        <div className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-2 pl-4 focus-within:ring-2 focus-within:ring-[#3A8FE0]/50 focus-within:bg-black/40 transition-all duration-300">
          <SearchIcon className="h-6 w-6 text-[#3A8FE0]" />
          <input
            ref={inputRef}
            type="text"
            value={query}
          ...
          {query && (
            <button
              onClick={() => setQuery("")}
              className="p-2 rounded-xl text-gray-500 hover:text-white hover:bg-white/10 transition-colors mr-1"
            >
      ...
      <div className="p-4 border-t border-white/5 flex justify-end">
        <button
          onClick={onClose}
          className="px-6 py-2.5 rounded-full text-sm font-bold text-gray-400 hover:text-white hover:bg-white/5 transition-colors"
        >
          Done
        </button>
      </div>
...
export const PlaylistQuickAdd: React.FC<PlaylistQuickAddProps> = ({
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className="relative bg-[#0B1E3D] rounded-[32px] shadow-[0_0_60px_rgba(58,143,224,0.15)] w-full max-w-2xl flex flex-col overflow-hidden transform transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] animate-in zoom-in-90 scale-100 border border-white/10"
