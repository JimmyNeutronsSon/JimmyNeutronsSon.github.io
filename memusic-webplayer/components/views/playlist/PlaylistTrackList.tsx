            ...
            {playlist.songs && playlist.songs.length > 0 ? (
                ...
                <div className="flex justify-between items-center text-gray-400 border-b border-white/10 pb-2 mb-2 px-4 text-sm uppercase font-semibold">
                    ...
                    <div className="relative" ref={sortMenuRef}>
                        ...
                        {isSortMenuOpen && (
                            <div className="absolute top-full right-0 mt-2 w-40 bg-[#163A6B] border border-white/10 rounded-lg shadow-2xl p-2 z-30">
                                <button onClick={() => {setSortKey('default'); setIsSortMenuOpen(false);}} className={`w-full text-left px-3 py-2 text-sm rounded-md hover:bg-white/10 ${sortKey === 'default' ? 'text-[#3A8FE0]' : ''}`}>Default</button>
                                <button onClick={() => {setSortKey('title'); setIsSortMenuOpen(false);}} className={`w-full text-left px-3 py-2 text-sm rounded-md hover:bg-white/10 ${sortKey === 'title' ? 'text-[#3A8FE0]' : ''}`}>Title</button>
                                <button onClick={() => {setSortKey('duration'); setIsSortMenuOpen(false);}} className={`w-full text-left px-3 py-2 text-sm rounded-md hover:bg-white/10 ${sortKey === 'duration' ? 'text-[#3A8FE0]' : ''}`}>Duration</button>
                            </div>
                        )}
                    </div>
