...
> = ({
  ...
  const renderSongItem = (song: Song) => {
    ...
    return (
      <div
        key={song.id}
        className="flex items-center p-2 rounded-lg hover:bg-white/10 transition-all duration-300 group animate-in fade-in slide-in-from-bottom-2"
      >
        <div className="w-10 h-10 rounded-md mr-3 flex-shrink-0 relative">
          {imageUrl ? (
            <img
              src={imageUrl}
              alt={song.name}
              className="w-full h-full object-cover rounded-md animate-image-appear"
              loading="lazy"
            />
          ) : (
          ...
          <button
            ...
            className={`absolute inset-0 bg-black/50 flex items-center justify-center text-white rounded-md transition-opacity ${isPreviewingThisSong ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
          >
            {isPreviewingThisSong && isPreviewPlaying ? (
    ...
    return (
      <div className="mt-12 pt-8 border-t border-white/10 text-center">
        <p className="text-gray-500 text-sm">
          Add some songs to start getting recommendations.
        </p>
      </div>
    );
  }
    ...
    <div className="mt-12 pt-8 border-t border-white/10">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Based on the vibes of this playlist.
          </p>
        </div>
        <button
        ...
        >
          <RefreshIcon
            className={`w-4 h-4 ${recsLoading ? "animate-spin" : ""}`}
          />
      ...
      {recsLoading && visibleRecommendations.length === 0 ? (
        ...
        <div className="text-center py-8 text-gray-500 text-sm">
          No new recommendations found. Try refreshing!
        </div>
      )}
    </div>
  );
