  ...
  switch (activeTab) {
    ...
    case "albums":
      return albums.length > 0 ? (
        <div
          className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 ${containerClass}`}
        >
        ...
        >
          {playlists.map((playlist, idx) => (
            ...
            >
              <PlaylistCard
                playlist={playlist}
                onClick={(p) => onResultClick("playlist", p)}
              />
            </div>
          ))}
        </div>
...
const SearchLanding: React.FC<{ onCategoryClick: (term: string) => void }> = ({
  onCategoryClick,
}) => {
  return (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {CATEGORIES.map((cat, idx) => (
          ...
          >
            ...
            <span className="absolute top-5 left-5 text-2xl font-black text-white tracking-tight drop-shadow-md z-10">
              {cat.label}
            </span>
...
const Search: React.FC<SearchProps> = ({
  ...
  const handleResultInteraction = useCallback(
    (
      ...
      if (type === "playlist" && typeof data !== "string")
        navigateToApiPlaylist(data);
    },
