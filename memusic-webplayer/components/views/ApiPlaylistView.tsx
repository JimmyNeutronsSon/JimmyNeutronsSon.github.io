...
const ApiPlaylistTrackItem: React.FC<{
    ...
    return (
        <div onClick={() => playSong(song, playlistSongs, { type: 'api_playlist', id: playlist.id })} className="grid grid-cols-[auto_1fr_auto_auto] items-center gap-x-4 p-3 px-4 rounded-xl hover:bg-white/10 cursor-pointer group transition-colors">
            <span className={`text-center w-6 font-medium ${isCurrent ? 'text-[#3A8FE0]' : 'text-gray-500'}`}>{index + 1}</span>
            <div>
                <p className={`font-semibold truncate text-base ${isCurrent ? 'text-[#3A8FE0]' : 'text-white'}`}>{song.name}</p>
...
const ApiPlaylistView: React.FC<ApiPlaylistViewProps> = ({ playlist, setActiveView, navigateToArtist }) => {
                ...
                content: (
                    <>
                        ...
                        <div className="flex justify-end space-x-4">
                            ...
                            <button onClick={() => { hideModal(); setActiveView('library'); }} className="px-4 py-2 rounded-lg bg-[#3A8FE0] text-white font-bold hover:bg-[#6CB4F0] transition-colors">{t('apiPlaylistView.viewInLibrary')}</button>
                        </div>
                    </>
                )
            });
    ...
    const handleDownloadAll = async () => {
                ...
                showModal({ 
                    ...
                    content: (
                        <div className="space-y-2">
                            ...
                            <div className="w-full bg-gray-600 rounded-full h-1"><div className="bg-[#3A8FE0] h-1 rounded-full" style={{ width: `${(filesAdded / totalFiles) * 100}%` }}></div></div>
                        </div>
                    ) 
                });
        ...
        zip.generateAsync({ type: "blob" }, (metadata: { percent: number }) => {
            showModal({ title: t('albumView.downloadCompressing'), content: <div className="space-y-2"><p>{t('albumView.downloadCompressingProgress', { percent: metadata.percent.toFixed(0) })}</p><div className="w-full bg-gray-600 rounded-full h-1"><div className="bg-[#3A8FE0] h-1 rounded-full" style={{ width: `${metadata.percent}%` }}></div></div></div> });
        }).then((content: Blob) => {
                            ...
                            <div className="flex items-center gap-x-6">
                                <div className="relative hidden sm:block">
                                    ...
                                    <SmartMenu isOpen={isSortMenuOpen} onClose={() => setIsSortMenuOpen(false)} triggerRef={sortMenuRef} width="w-48">
                                        <div className="flex flex-col py-1">
                                            <button onClick={() => {setSortKey('default'); setIsSortMenuOpen(false);}} className={`w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-white/10 transition-colors ${sortKey === 'default' ? 'text-[#3A8FE0] font-bold' : 'text-gray-300'}`}>{t('apiPlaylistView.sort_default')}</button>
                                            <button onClick={() => {setSortKey('title'); setIsSortMenuOpen(false);}} className={`w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-white/10 transition-colors ${sortKey === 'title' ? 'text-[#3A8FE0] font-bold' : 'text-gray-300'}`}>{t('apiPlaylistView.sort_title')}</button>
                                            <button onClick={() => {setSortKey('duration'); setIsSortMenuOpen(false);}} className={`w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-white/10 transition-colors ${sortKey === 'duration' ? 'text-[#3A8FE0] font-bold' : 'text-gray-300'}`}>{t('apiPlaylistView.sort_duration')}</button>
                                        </div>
                                    </SmartMenu>
                                </div>
