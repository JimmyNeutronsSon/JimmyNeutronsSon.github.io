...
const ArtistView: React.FC<ArtistViewProps> = ({
  ...
  const { playSong, isPlaying, togglePlay, contextId } =
    useContext(PlayerContext);
  const { isFavoriteArtist, toggleFavoriteArtist } =
  ...
  if (error || !artist)
    return (
      <div className="p-8 text-center text-gray-400">
        {error || "Artist not found or failed to load."}
      </div>
    );
  ...
  const bioText =
    artist.bio?.find((b) => b.title === "Bio")?.text ||
    artist.bio?.[0]?.text ||
    "";
  const shortBio =
    bioText.split(" ").slice(0, 40).join(" ") +
    (bioText.split(" ").length > 40 ? "..." : "");

  const metaString = t("artistView.followers", {
    count: parseInt(artist.fanCount || "0").toLocaleString(),
  });
              ...
              <div className="prose prose-invert max-w-none text-gray-300 leading-relaxed text-lg">
                ...
                {bioText.length > shortBio.length && (
                  <button
                    ...
                    className="text-[#3A8FE0] font-bold mt-2 hover:underline"
                  >
                    {showFullBio
                      ? t("artistView.showLess")
                      : t("artistView.readMore")}
                  </button>
                )}
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="text-xl font-bold text-white capitalize">
                    {artist.dominantType || artist.type}
                  </div>
