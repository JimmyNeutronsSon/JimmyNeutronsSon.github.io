...
const VerticalFader: React.FC<{
    ...
    return (
        <div className={`flex flex-col items-center h-full group select-none ${disabled ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
            ...
            <span className="text-[10px] font-mono text-[#3A8FE0] mt-1">{value > 0 ? `+${value.toFixed(1)}` : value.toFixed(1)}</span>
        </div>
    );
};
        ...
        <div className="space-y-8">
            <div className="flex flex-col items-center space-y-6">
                <div className="relative group">
                    ...
                    <button onClick={handleRandomAvatar} className="absolute top-0 right-0 transform translate-x-1/4 -translate-y-1/4 p-2.5 bg-[#2a2a2a] rounded-full text-white hover:bg-[#3a3a3a] hover:text-[#3A8FE0] border border-white/10 transition-colors shadow-lg z-10" title={t('settings.profile.random')}>
                        <DiceIcon className="w-4 h-4" />
                    </button>
                    <button onClick={() => setIsAvatarSelectorOpen(true)} className="absolute bottom-0 right-0 transform translate-x-1/4 translate-y-1/4 p-2.5 bg-[#3A8FE0] rounded-full text-black hover:bg-[#6CB4F0] transition-colors shadow-lg z-10" title={t('settings.profile.chooseAvatar')}>
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <div className="relative">
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl focus:outline-none focus:border-[#3A8FE0] focus:ring-1 focus:ring-[#3A8FE0] transition-all text-white text-lg font-medium placeholder-gray-600 pl-12" placeholder="Your Name" />
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={() => onSave(name, image)} disabled={!name.trim()} className="flex-1 px-4 py-3.5 rounded-xl bg-[#3A8FE0] text-white font-bold hover:bg-[#6CB4F0] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-[#3A8FE0]/20">{t('settings.profile.save')}</button>
                    </div>
                </div>
            )}
        </div>
                ...
                <div className="space-y-8">
                            ...
                            {[
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <span className={`font-bold text-lg ${selectedQuality === q.id ? 'text-[#3A8FE0]' : 'text-white'}`}>{q.label}</span>
                                        {selectedQuality === q.id && <div className="w-3 h-3 bg-[#3A8FE0] rounded-full shadow-[0_0_10px_#3A8FE0]" />}
                                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex p-1 bg-black/40 rounded-xl border border-white/5 relative max-w-sm">
                            <div className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-[#3A8FE0] rounded-lg transition-all duration-300 shadow-lg ${language === 'pt' ? 'left-[calc(50%+2px)]' : 'left-1'}`} />
                ...
                <div className="flex flex-wrap items-center justify-between gap-6 mb-10 relative z-10">
                    <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 shadow-inner bg-[#1a1a1a] border border-white/5`}>
                            <EqualizerIcon className={`w-6 h-6 transition-colors ${isEqEnabled ? 'text-[#3A8FE0]' : 'text-gray-600'}`} />
                        </div>
                        <div>
                            <h3 className="text-2xl font-black text-white tracking-tight">Master EQ</h3>
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full transition-all duration-300 ${isEqEnabled ? 'bg-[#3A8FE0] shadow-[0_0_8px_#3A8FE0]' : 'bg-[#333] shadow-none'}`} />
                    ...
                    <div className="flex items-center gap-4 bg-black/40 p-1.5 rounded-full border border-white/5 backdrop-blur-sm shadow-inner">
                        ...
                        <span className={`text-[10px] font-bold px-3 py-1 rounded-full transition-colors ${isEqEnabled ? 'text-[#3A8FE0]' : 'text-gray-500'}`}>ACTIVE</span>
                    </div>
                </div>
                ...
                <div className="relative z-10 flex flex-col gap-8">
                    <div className="flex items-center gap-5">
                        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#0A2D6E] to-[#1E6CC7] flex items-center justify-center shadow-lg shadow-blue-900/20 transform rotate-3 border border-white/10">
                            <span className="font-['Inter'] text-2xl font-extrabold text-white">W</span>
                        </div>
                        <div>
                            <h3 className="text-3xl font-black text-white tracking-tighter">Welkin.Music</h3>
