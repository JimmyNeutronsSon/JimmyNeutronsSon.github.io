...
const EqualizerIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    {...props}
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
...
const SettingsHeader: React.FC<{ title: string; description: string }> = ({
  title,
  description,
}) => (
  <div className="mb-8">
...
const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div
    className={`glass-panel border border-white/5 rounded-3xl p-6 md:p-8 ${className}`}
  >
    {children}
  </div>
);
...
const VerticalFader: React.FC<{
  ...
  return (
    <div
      className={`flex flex-col items-center h-full group select-none ${disabled ? "opacity-50 pointer-events-none grayscale" : ""}`}
    >
      <div
        ref={trackRef}
      ...
      >
        <div className="absolute w-1 h-[calc(100%-2rem)] top-4 bg-[#222] rounded-full shadow-[inset_0_1px_2px_rgba(0,0,0,1)]">
          <div
            className="absolute bottom-0 left-0 right-0 bg-[#3A8FE0] w-full rounded-b-full opacity-40"
        ...
        >
          ...
          <div className="absolute w-8 h-[2px] bg-[#3A8FE0] top-1/2 -translate-y-1/2 shadow-[0_0_4px_#3A8FE0]" />
        </div>
      </div>

      <span className="mt-4 text-[10px] font-bold text-gray-500 uppercase tracking-wider group-hover:text-white transition-colors">
        {label}
      </span>
      <span className="text-[10px] font-mono text-[#3A8FE0] mt-1">
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-4 sm:grid-cols-5 gap-4 max-h-[300px] overflow-y-auto custom-scrollbar p-1">
        <button
          ...
          className="aspect-square rounded-2xl bg-white/5 border-2 border-dashed border-white/10 hover:border-[#3A8FE0] hover:bg-white/10 flex flex-col items-center justify-center transition-all group"
          title={t("settings.profile.upload")}
        >
          <UploadIcon className="w-6 h-6 text-gray-500 group-hover:text-[#3A8FE0] transition-colors" />
          <input
            type="file"
            ref={fileInputRef}
        ...
        {defaultAvatars.map((url, index) => (
          <button
            ...
            className={`relative aspect-square rounded-2xl overflow-hidden border-2 transition-all duration-300 hover:scale-105 ${currentAvatar === url ? "border-[#3A8FE0] ring-2 ring-[#3A8FE0]/30" : "border-transparent hover:border-white/30"}`}
          >
            <img
              src={url}
              alt={`Avatar ${index + 1}`}
      ...
      <div className="flex justify-end pt-2">
        <button
          onClick={onClose}
          className="px-6 py-2 rounded-full bg-white/5 hover:bg-white/10 text-white font-medium transition-colors text-sm"
        >
    ...
    <div className="space-y-8">
      <div className="flex flex-col items-center space-y-6">
        <div className="relative group">
          <div className="w-32 h-32 rounded-full p-1 border-2 border-[#3A8FE0] shadow-[0_0_30px_rgba(58,143,224,0.3)] bg-[#0B1E3D] overflow-hidden">
            <img
              src={image}
              alt="Profile"
          ...
          <button
            ...
            className="absolute top-0 right-0 transform translate-x-1/4 -translate-y-1/4 p-2.5 bg-[#2a2a2a] rounded-full text-white hover:bg-[#3a3a3a] hover:text-[#3A8FE0] border border-white/10 transition-colors shadow-lg z-10"
            title={t("settings.profile.random")}
          >
            <DiceIcon className="w-4 h-4" />
          </button>
          <button
            onClick={() => setIsAvatarSelectorOpen(true)}
            className="absolute bottom-0 right-0 transform translate-x-1/4 translate-y-1/4 p-2.5 bg-[#3A8FE0] rounded-full text-black hover:bg-[#6CB4F0] transition-colors shadow-lg z-10"
            title={t("settings.profile.chooseAvatar")}
          >
      ...
      {isAvatarSelectorOpen ? (
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl focus:outline-none focus:border-[#3A8FE0] focus:ring-1 focus:ring-[#3A8FE0] transition-all text-white text-lg font-medium placeholder-gray-600 pl-12"
          ...
          <div className="flex gap-3">
            <button
              onClick={onCancel}
              className="flex-1 px-4 py-3.5 rounded-xl bg-white/5 hover:bg-white/10 text-white font-bold transition-colors"
            >
              {t("settings.profile.cancel")}
            </button>
            <button
              ...
              disabled={!name.trim()}
              className="flex-1 px-4 py-3.5 rounded-xl bg-[#3A8FE0] text-white font-bold hover:bg-[#6CB4F0] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-[#3A8FE0]/20"
            >
...
const SectionProfile: React.FC = () => {
  const { name, imageUrl, updateName, updateImage } =
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Card className="flex flex-col sm:flex-row items-center gap-8 relative overflow-hidden group">
        <div className="absolute inset-0 bg-gradient-to-r from-[#3A8FE0]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
        <div className="relative">
          <div className="w-32 h-32 rounded-full p-1 border-2 border-[#3A8FE0] shadow-[0_0_40px_rgba(58,143,224,0.3)] bg-[#0B1E3D] overflow-hidden group-hover:scale-105 transition-transform duration-500">
            <img
              src={imageUrl}
              alt={name}
              className="w-full h-full rounded-full object-cover"
        ...
        <div className="flex-1 text-center sm:text-left z-10">
          ...
          <button
            ...
            className="px-8 py-3 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 text-white font-bold transition-all hover:scale-105 hover:shadow-lg backdrop-blur-md"
          >
        ...
        <div className="space-y-8">
          <div>
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <span className="w-1 h-6 bg-[#3A8FE0] rounded-full block" />
              Streaming Quality
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                {
                  id: "160kbps",
                  label: t("settings.quality.normal"),
                  desc: t("settings.quality.normalDesc"),
                },
                {
                  id: "320kbps",
                  label: t("settings.quality.high"),
                  desc: t("settings.quality.highDesc"),
                },
              ].map((q) => (
                <button
                  key={q.id}
                  onClick={() => setSelectedQuality(q.id)}
                  className={`relative p-4 rounded-2xl border-2 text-left transition-all duration-300 ${selectedQuality === q.id ? "border-[#3A8FE0] bg-[#3A8FE0]/10" : "border-white/5 bg-white/5 hover:border-white/20"}`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span
                      className={`font-bold text-lg ${selectedQuality === q.id ? "text-[#3A8FE0]" : "text-white"}`}
                    >
                      {q.label}
                    </span>
                    {selectedQuality === q.id && (
                      <div className="w-3 h-3 bg-[#3A8FE0] rounded-full shadow-[0_0_10px_#3A8FE0]" />
                    )}
                  </div>
          ...
          <div>
            ...
            <div className="flex p-1 bg-black/40 rounded-xl border border-white/5 relative max-w-sm">
              <div
                className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-[#3A8FE0] rounded-lg transition-all duration-300 shadow-lg ${language === "pt" ? "left-[calc(50%+2px)]" : "left-1"}`}
              />
...
const SectionEffects: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <SettingsHeader
        title="Sonic Lab"
        description={t("settings.effects.description")}
      />

      <div className="glass-panel-heavy p-6 md:p-8 rounded-[32px] relative overflow-hidden border border-white/5 shadow-2xl bg-[#0B1E3D]">
        ...
        <div className="flex flex-wrap items-center justify-between gap-6 mb-10 relative z-10">
          <div className="flex items-center gap-4">
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 shadow-inner bg-[#1a1a1a] border border-white/5`}
            >
              <EqualizerIcon
                className={`w-6 h-6 transition-colors ${isEqEnabled ? "text-[#3A8FE0]" : "text-gray-600"}`}
              />
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full transition-all duration-300 ${isEqEnabled ? "bg-[#3A8FE0] shadow-[0_0_8px_#3A8FE0]" : "bg-[#333] shadow-none"}`}
                />
          ...
          <div className="flex items-center gap-4 bg-black/40 p-1.5 rounded-full border border-white/5 backdrop-blur-sm shadow-inner">
            <span
              className={`text-[10px] font-bold px-3 py-1 rounded-full transition-colors ${!isEqEnabled ? "bg-white/10 text-white" : "text-gray-500"}`}
            >
              BYPASS
            </span>
            <ToggleSwitch
              enabled={isEqEnabled}
              onChange={() => setIsEqEnabled(!isEqEnabled)}
            />
            <span
              className={`text-[10px] font-bold px-3 py-1 rounded-full transition-colors ${isEqEnabled ? "text-[#3A8FE0]" : "text-gray-500"}`}
            >
              ACTIVE
            </span>
          </div>
        ...
        <div className="relative z-10 transition-opacity duration-500">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-6 border-t border-white/5">
            <div className="flex gap-2 overflow-x-auto max-w-full pb-2 md:pb-0 custom-scrollbar-hidden mask-linear-fade">
              {(Object.keys(eqPresets) as Array<keyof typeof eqPresets>).map(
                (preset) => (
                  <button
                    key={preset}
                    onClick={() => {
            ...
            <button
              onClick={resetEq}
              className="text-[10px] font-bold text-red-400 hover:text-red-300 uppercase tracking-widest px-4 py-2 hover:bg-red-500/10 rounded transition-colors"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="glass-panel p-6 rounded-[24px] relative overflow-hidden group border border-white/5 bg-[#0B1E3D] shadow-2xl">
          ...
          <div className="flex items-center justify-between mb-6 relative z-10 px-2">
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${is8DEnabled ? "bg-[#00ff00] shadow-[0_0_8px_#00ff00]" : "bg-[#330000]"} transition-colors duration-200`}
              />
              <div>
            ...
            <ToggleSwitch
              enabled={is8DEnabled}
              onChange={() => setIs8DEnabled(!is8DEnabled)}
            />
          </div>
          ...
          <div className="relative h-32 bg-black rounded-lg border border-white/10 flex items-center justify-center overflow-hidden shadow-[inset_0_2px_10px_rgba(0,0,0,0.8)] mx-2">
            ...
            <div
              className={`absolute w-full h-full flex items-center justify-center transition-opacity duration-500 ${is8DEnabled ? "opacity-100" : "opacity-0"}`}
            >
        ...
        <div className="glass-panel p-6 rounded-[24px] relative overflow-hidden group border border-white/5 bg-[#0B1E3D] shadow-2xl">
          ...
          <div className="flex items-center justify-between mb-6 relative z-10 px-2">
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${isReverbEnabled ? "bg-[#00ff00] shadow-[0_0_8px_#00ff00]" : "bg-[#330000]"} transition-colors duration-200`}
              />
              <div>
                <span className="font-bold text-lg text-white block tracking-tight">
                  Verb Unit
            ...
            <ToggleSwitch
              ...
              onChange={() => setIsReverbEnabled(!isReverbEnabled)}
            />
          </div>
          ...
          <div className="relative h-16 bg-[#0a0a0a] rounded-lg border border-white/10 mb-5 overflow-hidden shadow-[inset_0_2px_5px_rgba(0,0,0,1)] mx-2">
            ...
            >
              ...
              <path
                ...
                fill={
                  isReverbEnabled ? "rgba(59, 130, 246, 0.2)" : "transparent"
                }
                className="transition-colors duration-300"
              />
            </svg>
            <div className="absolute top-1 right-2 font-mono text-[9px] text-blue-500/80">
              {isReverbEnabled
                ? `MIX: ${(reverbMix * 100).toFixed(0)}%`
                : "OFF"}
            </div>
          </div>

          <div
            className={`px-2 transition-all duration-300 ${isReverbEnabled ? "opacity-100" : "opacity-40 pointer-events-none grayscale"}`}
          >
            ...
            <div className="relative h-8 w-full flex items-center group/slider">
              ...
              <div className="w-full h-1.5 bg-[#0B1E3D] rounded-full border-b border-white/10 relative z-10 shadow-[inset_0_1px_2px_rgba(0,0,0,1)]"></div>
...
const SectionData: React.FC = () => {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `welkin_music_backup_${new Date().toISOString().split("T")[0]}.json`;
        ...
        content: (
          <div className="space-y-4">
            <p className="text-gray-300 text-sm leading-relaxed mb-4">
              {t("settings.data.importSubtitle")}
            </p>
            <div className="grid gap-3">
              <button
                onClick={() => processImport(jsonString, "merge")}
                className="flex items-center p-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-blue-500/50 transition-all text-left group"
              >
                <div className="p-3 rounded-xl bg-blue-500/20 text-blue-400 mr-4 group-hover:scale-110 transition-transform">
              ...
              <button
                onClick={() => processImport(jsonString, "replace")}
                className="flex items-center p-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-red-500/50 transition-all text-left group"
              >
                <div className="p-3 rounded-xl bg-red-500/20 text-red-400 mr-4 group-hover:scale-110 transition-transform">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
            ...
            <button
              onClick={hideModal}
              className="w-full py-3 rounded-xl text-gray-400 hover:text-white hover:bg-white/5 transition-colors text-xs font-bold uppercase tracking-widest"
            >
              {t("settings.profile.cancel")}
            </button>
          </div>
    ...
    showModal({
      ...
      content: (
        <div className="text-center space-y-6 py-4">
          <div
            className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center ${result.success ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
          >
            {result.success ? (
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <button
            onClick={hideModal}
            className="px-8 py-3 rounded-full bg-white/10 hover:bg-white/20 font-bold transition-colors text-xs uppercase tracking-widest"
          >
            OK
          </button>
        </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <button
          onClick={() => fileInputRef.current?.click()}
          className="group flex flex-col items-center justify-center p-8 rounded-3xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/20 hover:scale-[1.02] transition-all duration-300"
        >
          <div className="w-16 h-16 rounded-full bg-[#3A8FE0]/10 text-[#3A8FE0] flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-[0_0_20px_rgba(58,143,224,0.2)]">
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Card className="relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#0A2D6E]/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />

        <div className="relative z-10 flex flex-col gap-8">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#0A2D6E] to-[#1E6CC7] flex items-center justify-center shadow-lg shadow-blue-900/20 transform rotate-3 border border-white/10">
              <span className="font-['Inter'] text-2xl font-extrabold text-white">
                W
              </span>
            </div>
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                <span className="px-2 py-0.5 rounded-md bg-white/10 border border-white/10 text-xs font-bold text-[#0A2D6E] uppercase tracking-wider">
                  v1.2.0
                </span>
          ...
          <p className="text-gray-300 text-sm leading-relaxed max-w-2xl border-l-2 border-[#3A8FE0] pl-4">
            {t("settings.about.p1")}
          </p>
          ...
          <div>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-3 px-6 py-3.5 rounded-xl bg-white/5 hover:bg-white/10 text-white font-bold transition-all border border-white/10 hover:border-[#3A8FE0]/50 group"
            >
      ...
      {tabs.map((tab) => (
        <button
          ...
          className={`
                        ...
                        ${
                          activeTab === tab.id
                            ? "bg-[#3A8FE0] text-white shadow-[0_0_20px_rgba(58,143,224,0.3)] scale-[1.02]"
...
const Settings: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row overflow-hidden bg-[#0B1E3D]">
      <div className="w-full md:w-64 bg-[#0B1E3D]/50 backdrop-blur-xl border-b md:border-b-0 md:border-r border-white/5 z-20 flex-shrink-0">
        <div className="p-4 md:p-8 h-full flex flex-col">
          <h1 className="text-2xl font-black text-white tracking-tight mb-6 hidden md:block">
            Settings
          </h1>
