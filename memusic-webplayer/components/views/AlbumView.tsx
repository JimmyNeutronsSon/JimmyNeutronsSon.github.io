  ...
  if (error || !album)
    return (
      <div className="p-8 text-center text-gray-400">
        {error || "Album not found or failed to load."}
      </div>
    );
    ...
    zip
      ...
      .catch((err: any) => {
        ...
        showModal({
          ...
          content: (
            <>
              <p className="text-gray-300 mb-6">{t("albumView.errorZip")}</p>
              <div className="flex justify-end">
                <button
                  onClick={hideModal}
                  className="px-4 py-2 rounded-md bg-white/10 hover:bg-white/20"
      ...
      {album.artists.primary.map((a, i) => (
        <React.Fragment key={a.id}>
          <span
            ...
            className="hover:text-white hover:underline cursor-pointer transition-colors font-bold"
          >
            {a.name}
          </span>
