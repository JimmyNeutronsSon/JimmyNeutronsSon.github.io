...
const AmbientBackground = React.memo(({ imageUrl }: { imageUrl: string }) => {
  return (
    <div className="absolute inset-0 z-0 select-none pointer-events-none overflow-hidden bg-[#0B1E3D] transition-colors duration-1000">
      <div
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_10%,#0B1E3D_130%)] opacity-80" />
...
const HeaderControls = React.memo(
  ({
    ...
    <div
      className={`flex items-center gap-2 md:gap-3 z-[100] absolute top-4 right-4 md:top-6 md:right-6 transition-opacity duration-500 ${visible ? "opacity-100" : "opacity-0 pointer-events-none"}`}
    >
      <button
      ...
      >
        {isFullscreen ? (
          <MinimizeIcon className="w-4 h-4 md:w-5 md:h-5" />
        ) : (
          <MaximizeIcon className="w-4 h-4 md:w-5 md:h-5" />
        )}
      </button>
      <button
        onClick={onClose}
...
const ReactiveSettingsMenu = ({
    ...
    const animate = () => {
      ...
      const bass =
        dataArray
          ...
          .reduce((a, b) => a + b, 0) / Math.floor(bufferLength * 0.05) || 0;
      const mids =
        dataArray
          .slice(Math.floor(bufferLength * 0.2), Math.floor(bufferLength * 0.5))
          .reduce((a, b) => a + b, 0) / Math.floor(bufferLength * 0.3) || 0;
      const treble =
        dataArray
          .slice(Math.floor(bufferLength * 0.5), bufferLength)
          .reduce((a, b) => a + b, 0) /
          (bufferLength - Math.floor(bufferLength * 0.5)) || 0;
      ...
      smoothedTreble.current +=
        (treble - smoothedTreble.current) * smoothingFactor;
      ...
      stars.current.forEach((star) => {
        ...
        if (px > 0 && px < canvas.width && py > 0 && py < canvas.height) {
          ...
          const alpha =
            ...
            (0.3 + (smoothedTreble.current / 255) * 0.7);
      ...
      const baseRadius =
        canvas.width / 7 + (smoothedBass.current / 255) * (canvas.width / 10);
      ...
      gradient.addColorStop(0.6, "#3A8FE0");
      gradient.addColorStop(
        1,
        `hsl(25, 100%, ${30 + (smoothedBass.current / 255) * 10}%)`,
      );
      ...
      for (let i = 0; i <= points; i++) {
        ...
        const midOffset =
          ...
          (smoothedMids.current / 255) *
          (canvas.width / 25);
        const trebleIndex =
          Math.floor((i / points) * (bufferLength * 0.5)) +
          Math.floor(bufferLength * 0.5);
        const spike =
          (dataArray[trebleIndex] / 255) *
          (canvas.width / 8) *
          (smoothedTreble.current / 255);
      ...
      const avgBass =
        bassHistory.current.reduce((a, b) => a + b, 0) /
        bassHistory.current.length;
      const kickThreshold = 1.25;
      const cooldown = 120;
      const now = Date.now();

      if (
        bass > avgBass * kickThreshold &&
        now - lastKickTime.current > cooldown
      ) {
  ...
  return (
    <div
      className={`fixed top-6 left-6 z-50 group flex flex-col gap-2 transition-opacity duration-500 ${visible ? "opacity-100" : "opacity-0 pointer-events-none"}`}
    >
      <div className="relative bg-black/40 backdrop-blur-2xl border border-white/10 rounded-full transition-all duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] overflow-hidden w-10 h-10 md:w-12 md:h-12 group-hover:h-[9.5rem] shadow-2xl flex flex-col items-center">
        <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center shrink-0 z-20 cursor-pointer relative">
          <div
            className={`absolute w-3 h-3 bg-[#3A8FE0] rounded-full shadow-[0_0_10px_#3A8FE0] transition-all duration-700 ease-in-out ${isPlaying ? "opacity-0 scale-150 blur-md" : "opacity-100 scale-100 blur-0"}`}
          />
          <canvas
            ref={canvasRef}
            className={`absolute inset-0 w-full h-full transition-all duration-700 ease-in-out ${isPlaying ? "opacity-100 scale-100" : "opacity-0 scale-0"}`}
          />
        </div>
        <div className="flex flex-col gap-3 items-center w-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 pt-1 pb-4 z-10">
          <button
            onClick={toggle8D}
            title="8D Audio"
            className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 border ${is8D ? "bg-[#3A8FE0] border-[#3A8FE0] text-white shadow-[0_0_15px_rgba(58,143,224,0.6)]" : "bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 hover:border-white/30"}`}
          >
            <HeadphonesIcon className="w-5 h-5 fill-none" />
          </button>
          <button
            onClick={handleReverbClick}
            title="Reverb"
            className={`relative w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 border ${isReverb ? "bg-[#3A8FE0] border-[#3A8FE0] text-white shadow-[0_0_15px_rgba(58,143,224,0.6)]" : "bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 hover:border-white/30"}`}
          >
            ...
            {isReverb && (
              <span className="absolute -right-1 -bottom-1 flex gap-0.5">
                <span
                  className={`w-1 h-1 rounded-full bg-white ${reverbMix >= 0.2 ? "opacity-100" : "opacity-30"}`}
                />
                <span
                  className={`w-1 h-1 rounded-full bg-white ${reverbMix >= 0.5 ? "opacity-100" : "opacity-30"}`}
                />
                <span
                  className={`w-1 h-1 rounded-full bg-white ${reverbMix >= 0.8 ? "opacity-100" : "opacity-30"}`}
                />
              </span>
            )}
          </button>
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-md animate-in fade-in slide-in-from-left-8 duration-700 mx-auto">
      ...
      <div className="space-y-1">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2
              className="text-3xl font-black text-white leading-tight line-clamp-2"
          ...
          <button
            ...
            className={`p-3 rounded-full transition-colors ${isFavorite ? "text-[#3A8FE0] bg-[#3A8FE0]/10" : "text-gray-400 hover:text-white hover:bg-white/10"}`}
          >
            {isFavorite ? (
      ...
      <div className="space-y-4">
        ...
        >
          <div
            className="absolute h-full bg-[#3A8FE0] rounded-full group-hover/bar:bg-[#6CB4F0] transition-all shadow-[0_0_10px_#3A8FE0]"
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          className={`p-2 transition-colors ${isShuffle ? "text-[#3A8FE0]" : "text-gray-500 hover:text-white"}`}
        >
        ...
        <button
          ...
          className={`p-2 transition-colors relative ${repeatMode !== "off" ? "text-[#3A8FE0]" : "text-gray-500 hover:text-white"}`}
        >
          <RepeatIcon className="w-5 h-5" />
          {repeatMode === "one" && (
            <span className="absolute top-0 right-0 text-[8px] font-bold bg-[#3A8FE0] text-white px-0.5 rounded">
              1
            </span>
          )}
        </button>
  ...
  <div className="h-full overflow-y-auto custom-scrollbar px-6 md:px-12 py-20 text-center">
    <div className="space-y-6 max-w-2xl mx-auto">
      {lines.map((line, idx) => (
        <p
          key={idx}
          className="text-xl md:text-2xl text-gray-300 font-medium leading-relaxed hover:text-white transition-colors"
        >
          {line}
        </p>
      ))}
    </div>
    <p className="mt-12 text-sm text-gray-600 font-bold uppercase tracking-widest">
      Lyrics not synced
    </p>
  </div>
);
...
export const LyricsOverlay: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<"embedded" | "fullscreen" | "art">(
    "embedded",
  );
  ...
  return (
    <div
      className={`${containerStyle} flex flex-col transition-all duration-500 overflow-hidden`}
    >
      ...
      <HeaderControls
        ...
        onToggleFullscreen={() =>
          setViewMode((prev) =>
            prev === "embedded" ? "fullscreen" : "embedded",
          )
        }
      ...
      <div
        className={`relative z-10 flex w-full h-full ${isFullscreen ? "flex-col lg:flex-row" : "flex-col"}`}
      >
        {isFullscreen && (
          <div
            className={`
        ...
        <div
          className={`relative flex-1 flex flex-col min-h-0 ${isFullscreen ? (viewMode === "art" ? "hidden" : "lg:w-[55%] xl:w-[60%] w-full") : "w-full"}`}
        >
          {isFullscreen && !viewMode && (
            <div
              className={`flex-shrink-0 text-center px-6 transition-all duration-500 lg:hidden pt-20 pb-4`}
            >
              ...
              <h2 className="text-white font-black text-2xl truncate tracking-tight">
                {currentSong?.name}
              </h2>
