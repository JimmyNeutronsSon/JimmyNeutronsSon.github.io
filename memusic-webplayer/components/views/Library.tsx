...
const CreatePlaylistCard: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <div
    onClick={onClick}
    className="group relative aspect-square rounded-3xl bg-white/5 border-2 border-dashed border-white/10 hover:border-[#3A8FE0] hover:bg-white/10 transition-all duration-300 cursor-pointer flex flex-col items-center justify-center gap-4 overflow-hidden"
  >
    <div className="p-4 rounded-full bg-white/5 group-hover:bg-[#3A8FE0] transition-colors duration-300 group-hover:scale-110 shadow-lg">
      <PlusIcon className="w-8 h-8 text-gray-400 group-hover:text-black transition-colors" />
    </div>
    <h4 className="font-bold text-gray-400 group-hover:text-white transition-colors tracking-wide text-sm uppercase">
      Create Playlist
    </h4>
  </div>
);

const LocalPlaylistCard: React.FC<{
        ...
        <>
          ...
          <div className="flex justify-end space-x-4">
            ...
            <button
              ...
              className="px-4 py-2 rounded-md bg-red-600 text-white font-bold hover:bg-red-500"
            >
              Delete
            </button>
          </div>
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-square mb-4 overflow-hidden rounded-2xl shadow-lg bg-[#0B1E3D]">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={playlist.name}
        ...
        <button
          onClick={handlePlay}
          className="absolute bottom-4 right-4 w-12 h-12 bg-[#3A8FE0] rounded-full flex items-center justify-center text-black shadow-lg shadow-black/40 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 ease-out hover:scale-105 active:scale-95"
        >
      ...
      <div className="flex justify-between items-start relative">
        <div className="flex-1 min-w-0 pr-2">
          <h4 className="font-bold text-white truncate text-lg tracking-tight group-hover:text-[#3A8FE0] transition-colors">
            {playlist.name}
          </h4>
        ...
        <div className="relative" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div className="absolute top-full right-0 mt-1 w-40 bg-[#163A6B] border border-white/10 rounded-lg shadow-2xl p-1 z-20 animate-in fade-in zoom-in-95">
              <button
                onClick={handleDelete}
                className="w-full flex items-center gap-2 text-left px-3 py-2 text-sm rounded-md text-red-400 hover:bg-white/10 hover:text-red-300"
              >
...
const EmptyState: React.FC<{
  ...
  <div className="flex flex-col items-center justify-center py-24 text-center animate-in fade-in zoom-in-95 duration-500">
    ...
    {actionText && onAction && (
      <button
        onClick={onAction}
        className="px-8 py-3 rounded-full bg-[#3A8FE0] text-white font-bold hover:bg-[#6CB4F0] transition-colors shadow-lg shadow-[#3A8FE0]/20"
      >
        {actionText}
      </button>
    )}
...
const PlaylistsView: React.FC<{
  onCreate: () => void;
  onNavigate: NavFunc;
  onApiNavigate: NavFunc;
}> = ({ onCreate, onNavigate, onApiNavigate }) => {
      ...
      {favoriteApiPlaylists.length > 0 && (
        <section>
          <SectionHeader
            ...
            count={favoriteApiPlaylists.length}
          />
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {favoriteApiPlaylists.map((p) => (
              <div
                key={p.id}
                className="group relative bg-white/5 p-4 rounded-3xl border border-white/5 hover:border-white/10 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl"
              >
                <ApiPlaylistCard
                  playlist={p}
                  onClick={() => onApiNavigate(p)}
                />
              </div>
            ))}
          </div>
...
const SongsView: React.FC<{
  onNavigateArtist: NavFunc;
  onNavigatePlaylist: NavFunc;
}> = ({ onNavigateArtist, onNavigatePlaylist }) => {
  const { favoriteSongs } = useContext(UserMusicContext);
  const [sortKey, setSortKey] = useState<
    "date_added" | "title_asc" | "title_desc"
  >("date_added");
  ...
  return (
    <section className="animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="absolute top-full right-0 mt-2 w-48 bg-[#1c1c1c] border border-white/10 rounded-xl shadow-2xl p-1 z-30 animate-in fade-in zoom-in-95">
              {sortOptions.map((opt) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 text-sm rounded-lg transition-colors ${sortKey === opt.key ? "bg-[#3A8FE0] text-white font-bold" : "text-gray-300 hover:bg-white/10"}`}
                >
                  {opt.label}
                </button>
              ))}
      ...
      <div className="bg-black/20 rounded-3xl border border-white/5 p-2 md:p-6">
        <SongList
          songs={sortedSongs}
          navigateToArtist={onNavigateArtist}
          navigateToPlaylist={onNavigatePlaylist}
...
const AlbumsView: React.FC<{
  onNavigateAlbum: NavFunc;
  onNavigateArtist: NavFunc;
}> = ({ onNavigateAlbum, onNavigateArtist }) => {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {favoriteAlbums.map((a) => (
          ...
          >
            <AlbumCard
              album={a}
              onAlbumClick={onNavigateAlbum}
              onArtistClick={onNavigateArtist}
            />
          </div>
        ))}
      </div>
...
const AllView: React.FC<{
  ...
  onNavigateArtist: NavFunc;
  onCreate: () => void;
}> = ({ onNavigatePlaylist, onNavigateAlbum, onNavigateArtist, onCreate }) => {
  const { playlists, favoriteAlbums, favoriteArtists, favoriteSongs } =
  ...
  if (!hasContent)
    return (
      <EmptyState
        ...
        onAction={() => {}}
      />
    ); // Action handled by parent if needed, simplistic here
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          ...
          {playlists.slice(0, 5).map((p) => (
            <LocalPlaylistCard
              key={p.id}
              playlist={p}
              onClick={() => onNavigatePlaylist(p.id)}
            />
          ))}
        </div>
      </section>
...
const Library: React.FC<LibraryProps> = ({
  ...
  const { createPlaylist, playlists, favoriteSongs } =
    useContext(UserMusicContext);
  const { showModal, hideModal } = useContext(ModalContext);
  const { t } = useTranslation();
    ...
    showModal({
      ...
      content: (
        <CreatePlaylistForm
          onCancel={hideModal}
          onConfirm={(name, desc) => {
            const newP = createPlaylist(name, desc);
            hideModal();
            navigateToPlaylist(newP.id);
          }}
        />
      ),
    });
  ...
  const renderContent = () => {
    switch (activeFilter) {
      case "playlists":
        return (
          <PlaylistsView
            ...
            onNavigate={navigateToPlaylist}
            onApiNavigate={navigateToApiPlaylist}
          />
        );
      case "songs":
        return (
      ...
      case "artists":
        return <ArtistsView onNavigate={navigateToArtist} />;
      case "albums":
        return (
          <AlbumsView
