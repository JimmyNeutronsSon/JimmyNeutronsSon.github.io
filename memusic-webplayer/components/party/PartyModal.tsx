...
const JoinView: React.FC<JoinViewProps> = ({
  ...
  const [code, setCode] = useState<string[]>(
    initialCode ? initialCode.split("") : Array(5).fill(""),
  );
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out">
      <h2 className="text-3xl font-black mb-3 text-center tracking-tight text-white">
        {t("partyModal.joinTitle")}
      </h2>
      <p className="mb-8 text-center text-gray-400 text-sm">
        {t("partyModal.joinSubtitle")}
      </p>
        ...
        {Array.from({ length: 5 }).map((_, index) => (
          <input
            ...
            className={`w-12 h-16 sm:w-14 sm:h-20 bg-black/20 border-2 rounded-lg text-center text-3xl font-bold uppercase transition-all duration-200 caret-transparent focus:outline-none
                            ${code[index] ? "border-[#3A8FE0] text-[#3A8FE0] bg-[#3A8FE0]/5" : "border-white/10 text-white focus:border-white/30 focus:bg-white/5"}`}
      ...
      <div className="flex flex-col-reverse sm:flex-row gap-3 justify-between items-center">
        ...
        <button
          ...
          className="w-full sm:w-auto px-10 py-3 rounded-xl bg-[#3A8FE0] text-white font-bold hover:bg-[#6CB4F0] disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-[#3A8FE0]/20 hover:shadow-[#3A8FE0]/40 text-sm transform active:scale-95 duration-100 flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
...
const CreateView: React.FC<{
  setView: (view: "landing" | "share") => void;
    ...
    return (
      <div className="animate-in fade-in zoom-in-95 duration-300 ease-out text-center flex flex-col items-center">
        ...
        <p className="text-gray-400 mb-8 text-sm leading-relaxed max-w-xs">
          Your network seems to be restricting peer-to-peer connections. This is
          common with VPNs or corporate firewalls.
        </p>
        <div className="flex gap-4 w-full justify-center">
          ...
          <button
            ...
            className="px-8 py-3 rounded-xl bg-[#3A8FE0] text-white font-bold hover:bg-[#6CB4F0] shadow-lg shadow-[#3A8FE0]/20 hover:shadow-[#3A8FE0]/40 transition-all text-sm"
          >
            Retry
          </button>
        </div>
  ...
  const OptionCard: React.FC<{
    ...
    <button
      ...
      className={`group relative flex flex-col items-center text-center p-6 rounded-xl border-2 transition-all duration-200 ease-out focus:outline-none w-full h-full
             ${
               isSelected
                 ? "border-[#3A8FE0] bg-[#3A8FE0]/10 shadow-[0_0_20px_rgba(58,143,224,0.1)]"
                 : "border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20"
             }`}
    >
      <div
        className={`absolute top-3 right-3 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-200 ${isSelected ? "scale-100 opacity-100" : "scale-0 opacity-0"}`}
      >
        <div className="w-full h-full bg-[#3A8FE0] rounded-full flex items-center justify-center shadow-md">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            fill="currentColor"
      ...
      <div
        className={`w-16 h-16 mb-5 flex items-center justify-center rounded-2xl transition-colors duration-200 ${isSelected ? "bg-[#3A8FE0] text-white" : "bg-white/10 text-gray-400 group-hover:text-white"}`}
      >
        {React.cloneElement(
      ...
      <h4
        className={`font-bold text-lg mb-2 transition-colors duration-200 ${isSelected ? "text-white" : "text-gray-200 group-hover:text-white"}`}
      >
        {title}
      </h4>
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out">
      <h2 className="text-3xl font-black mb-3 text-center tracking-tight text-white">
        {t("partyModal.createTitle")}
      </h2>
      <p className="mb-8 text-center text-gray-400 text-sm">
        {t("partyModal.createSubtitle")}
      </p>
      ...
      <div className="flex flex-col-reverse sm:flex-row gap-3 justify-between items-center">
        ...
        <button
          onClick={handleCreate}
          disabled={creating}
          className="w-full sm:w-auto px-10 py-3 rounded-xl bg-[#3A8FE0] text-white font-bold hover:bg-[#6CB4F0] shadow-lg shadow-[#3A8FE0]/20 hover:shadow-[#3A8FE0]/40 transition-all text-sm transform active:scale-95 duration-100 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {creating ? (
            <>
              <span className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" />
...
const ShareView: React.FC<{ partyId: string; onClose: () => void }> = ({
  partyId,
  onClose,
}) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out text-center">
      <div className="mx-auto mb-6 w-20 h-20 rounded-full bg-[#3A8FE0]/10 flex items-center justify-center border border-[#3A8FE0]/20 animate-bounce-small">
        <ShareIcon className="w-10 h-10 text-[#3A8FE0]" />
      </div>
      <h2 className="text-3xl font-black mb-2 tracking-tight text-white">
        {t("partyModal.shareTitle")}
      </h2>
      <p className="text-gray-400 mb-8 text-sm max-w-xs mx-auto leading-relaxed">
        {t("partyModal.shareSubtitle")}
      </p>

      <div className="mb-8">
        <div className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">
          {t("partyModal.partyCode")}
        </div>
        <div
          onClick={() => handleCopy(partyId, "code")}
          className="group relative inline-block bg-[#1a1a1a] px-10 py-5 rounded-2xl cursor-pointer border border-white/10 hover:border-[#3A8FE0]/50 transition-all duration-200 active:scale-95 shadow-inner"
        >
          <p className="font-mono text-5xl sm:text-6xl font-bold tracking-widest text-white drop-shadow-md">
            {partyId}
          </p>

          <div
            className={`absolute inset-0 bg-[#3A8FE0] rounded-2xl flex items-center justify-center text-black font-bold text-lg transition-opacity duration-200 ${copied === "code" ? "opacity-100" : "opacity-0 pointer-events-none"}`}
          >
      ...
      <div className="flex justify-center mb-8">
        ...
        >
          ...
          {copied === "link"
            ? t("partyModal.linkCopied")
            : t("partyModal.copyInvite")}
        </button>
      </div>
      ...
      <div className="pt-6 border-t border-white/5">
        <button
          onClick={onClose}
          className="text-gray-500 hover:text-white font-medium text-sm transition-colors px-6 py-2"
        >
          {t("partyModal.done")}
        </button>
      </div>
...
const LandingView: React.FC<{ setView: (view: "create" | "join") => void }> = ({
  setView,
}) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out">
      <h2 className="text-4xl font-black mb-4 text-center tracking-tighter text-white">
        {t("partyModal.landingTitle")}
      </h2>
      <p className="text-gray-400 mb-10 text-center max-w-xs mx-auto leading-relaxed text-sm">
        {t("partyModal.landingSubtitle")}
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button
          onClick={() => setView("create")}
          className="flex flex-col items-center p-8 rounded-3xl bg-[#163A6B] border border-white/5 hover:border-[#3A8FE0]/50 hover:bg-[#2f2f2f] transition-all duration-300 group text-center transform active:scale-[0.98] shadow-lg hover:shadow-xl"
        >
          <div className="w-16 h-16 rounded-2xl bg-black/40 flex items-center justify-center mb-5 border border-white/5 group-hover:border-[#3A8FE0]/30 transition-colors duration-300">
            <BroadcastIcon className="w-8 h-8 text-[#3A8FE0]" />
          </div>
          <h3 className="font-bold text-xl text-white mb-2">
            {t("partyModal.createCardTitle")}
          </h3>
          <p className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">
            {t("partyModal.createCardDesc")}
          </p>
        </button>
        <button
          onClick={() => setView("join")}
          className="flex flex-col items-center p-8 rounded-3xl bg-[#163A6B] border border-white/5 hover:border-white/30 hover:bg-[#2f2f2f] transition-all duration-300 group text-center transform active:scale-[0.98] shadow-lg hover:shadow-xl"
        >
          ...
          <h3 className="font-bold text-xl text-white mb-2">
            {t("partyModal.joinCardTitle")}
          </h3>
          <p className="text-sm text-gray-400 group-hover:text-gray-300 transition-colors">
            {t("partyModal.joinCardDesc")}
          </p>
        </button>
      </div>
    </div>
...
export const PartyModal: React.FC<PartyModalProps> = ({ onClose }) => {
  const [view, setView] = useState<"landing" | "create" | "join" | "share">(
    "landing",
  );
  ...
  return <div className="w-full max-w-xl p-2">{renderView()}</div>;
};
