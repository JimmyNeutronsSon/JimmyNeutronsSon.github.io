...
const GlobalSearchBar: React.FC<{
  ...
  return (
    ...
    >
      <div
        className={`relative transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] transform ${isFocused ? "scale-[1.02]" : "scale-100"}`}
      >
        <div
          className={`absolute -inset-[1px] bg-gradient-to-r from-[#3A8FE0] via-[#6CB4F0] to-[#3A8FE0] rounded-2xl blur-md transition-opacity duration-700 ease-in-out ${isFocused ? "opacity-30" : "opacity-0"}`}
        ></div>
        ...
        <div className="relative">
          ...
          />
          <div
            className={`absolute inset-y-0 left-0 flex items-center ${isMini ? "pl-3" : "pl-5"} pointer-events-none transition-colors duration-300 ${isFocused ? "text-[#3A8FE0]" : "text-gray-400"}`}
          >
      ...
      {isFocused && query === "" && searchHistory.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-4 bg-[#0B1E3D]/95 backdrop-blur-3xl border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.6)] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="max-h-80 overflow-y-auto custom-scrollbar p-2">
            {searchHistory.map((item, index) => (
              <div
                ...
                className="group/item flex items-center justify-between w-full text-left px-4 py-3.5 rounded-xl text-gray-300 hover:bg-white/5 hover:text-white cursor-pointer transition-all duration-200"
              >
                <div className="flex items-center gap-4 overflow-hidden">
                  <div className="p-2 rounded-lg bg-white/5 text-gray-500 group-hover/item:text-[#3A8FE0] group-hover/item:bg-[#3A8FE0]/10 transition-colors">
                ...
                <button
                  ...
                  className="p-2 rounded-full opacity-0 group-hover/item:opacity-100 text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-all transform hover:scale-110"
                >
    ...
    <header className="h-24 bg-[#0B1E3D]/80 backdrop-blur-xl px-4 flex items-center gap-6 z-40 flex-shrink-0 border-b border-white/5 md:grid md:grid-cols-[16rem_1fr_auto] md:px-8 shadow-sm">
      ...
      <div className="hidden md:flex justify-end items-center">
        ...
        <button
          ...
          className="ml-8 h-12 w-12 rounded-full p-[2px] bg-gradient-to-tr from-[#3A8FE0] to-purple-600 hover:scale-105 transition-transform duration-300 shadow-lg shadow-[#3A8FE0]/20"
          aria-label="Profile and settings"
        >
          <div className="h-full w-full rounded-full p-[2px] bg-[#0B1E3D]">
            <img
              src={imageUrl}
              alt="Profile"
