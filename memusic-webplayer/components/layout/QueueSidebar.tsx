...
const QueueItem: React.FC<QueueItemProps> = React.memo(
  ({ song, isPlaying, onPlay, navigateToArtist, addedBy, isGhost = false }) => {
    const { removeSongFromQueue, moveSongInQueue } = useContext(PlayerContext);
    const { partyState, myId, isHost, removeSongFromPartyQueue } =
    ...
    const canRemove =
      isHost ||
      (partyState?.mode === "collaborative" &&
        (song as PartyQueueSong).addedBy === myId);
      ...
      >
        <div className="relative flex-shrink-0 w-10 h-10 mr-3">
          <img
            src={imageUrl}
            alt={song.name}
            className="w-full h-full rounded-md object-cover animate-image-appear"
            loading="lazy"
          />
          {addedBy && (
            <img
              ...
              className="w-5 h-5 rounded-full absolute -bottom-1 -right-1 border-2 border-[#163A6B] shadow-md"
            />
          )}
        </div>
        <div className="flex-1 min-w-0">
          <p
            className={`font-semibold truncate ${isPlaying ? "text-[#3A8FE0]" : "text-white"}`}
          >
            {song.name}
          </p>
        ...
        {!isGhost && (!partyState || canRemove) && (
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="p-2 rounded-full hover:bg-white/20 text-gray-400 hover:text-white"
            >
              <MoreIcon className="w-5 h-5" />
            </button>
            {isMenuOpen && (
              <div className="absolute bottom-full right-0 mb-2 w-48 bg-[#163A6B] border border-white/10 rounded-lg shadow-2xl p-2 z-50">
                {!partyState && (
                  <button
                    onClick={() =>
                      handleMenuAction(() => moveSongInQueue(song.id, "top"))
                    }
                    className="w-full text-left px-3 py-2 text-sm rounded-md hover:bg-white/10"
                  >
                    {t("queue.moveToTop")}
                  </button>
                )}
                {!partyState && (
                  <button
                    onClick={() =>
                      handleMenuAction(() => moveSongInQueue(song.id, "bottom"))
                    }
                    className="w-full text-left px-3 py-2 text-sm rounded-md hover:bg-white/10"
                  >
                    {t("queue.moveToBottom")}
                  </button>
                )}
                <button
                  onClick={() =>
                    handleMenuAction(() =>
                      partyState
                        ? removeSongFromPartyQueue(song.id)
                        : removeSongFromQueue(song.id),
                    )
                  }
                  className="w-full text-left px-3 py-2 text-sm rounded-md hover:bg-white/10 text-red-400 hover:text-red-300"
                >
                  {t("queue.removeFromQueue")}
                </button>
              </div>
            )}
          </div>
...
const DraggableQueueItem: React.FC<DraggableQueueItemProps> = React.memo(
  ({
    song,
    index,
    isNowPlaying = false,
    ...
    return (
      ...
      >
        ...
        />
        <div
          className={`transition-opacity duration-200 ${isBeingDragged ? "opacity-40" : "opacity-100"}`}
        >
          <QueueItem
            song={song}
...
const QueueView: React.FC<{ navigateToArtist: (artistId: string) => void }> = ({
  navigateToArtist,
}) => {
  const {
    currentQueue,
  ...
  const playContext = partyState
    ? { type: "party" as const, id: partyState.partyId }
    : { type: contextType || "queue", id: contextId || song?.id || "" };

  const currentSongIndex = song ? queue.findIndex((s) => s.id === song.id) : -1;

  const upNextSongs =
    currentSongIndex !== -1 ? queue.slice(currentSongIndex + 1) : queue;
  const previouslyPlayedSongs =
    currentSongIndex !== -1 ? queue.slice(0, currentSongIndex) : [];
  ...
  const handleDrop = (index: number) => {
    ...
    partyState
      ? reorderPartyQueue(draggedIndex, index)
  ...
  const renderDraggableItem = (
    songItem: PartyQueueSong | Song,
    index: number,
    isNowPlaying = false,
  ) => {
    const addedBy =
      partyState && "addedBy" in songItem
        ? partyState.participants.find(
            (p) => p.id === (songItem as PartyQueueSong).addedBy,
          )
        : undefined;
    return (
      <DraggableQueueItem
        {...commonDragProps}
        onPlay={() => handlePlaySongFromQueue(songItem)}
        song={songItem}
        index={index}
  ...
  const AutoplayBanner = () => (
    <div className="my-3 p-3 bg-white/5 border border-white/10 rounded-lg flex items-center space-x-3 animate-in fade-in">
      <div className="p-2 bg-[#3A8FE0]/20 rounded-full flex-shrink-0">
        <SignalIcon className="w-5 h-5 text-[#3A8FE0]" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-bold text-white text-sm">{t("queue.autoplay")}</p>
        <p className="text-[10px] md:text-xs text-gray-400 leading-tight">
          {t("queue.autoplaySubtitle")}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar pr-1">
      {song && currentSongIndex !== -1 && (
        <div className="mb-4">
          <p className="text-sm font-bold uppercase text-gray-400 mb-2 px-2">
            {t("queue.nowPlaying")}
          </p>
      ...
      {upNextSongs.length > 0 && (
        <div className="mb-4">
          ...
          {upNextSongs.map((s, i) => {
            ...
            const isFirstAutoplaySong =
              !partyState &&
              autoplayStartIndex !== null &&
              originalIndex === autoplayStartIndex;
            return (
          ...
          <div className="flex justify-between items-center mb-6 flex-shrink-0">
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="relative text-xs font-bold uppercase bg-[#3A8FE0]/20 text-[#3A8FE0] px-3 py-1 rounded-md hover:bg-[#3A8FE0]/30 transition-all duration-200 border border-[#3A8FE0]/20"
                style={{ minWidth: "70px" }}
              >
                <span
