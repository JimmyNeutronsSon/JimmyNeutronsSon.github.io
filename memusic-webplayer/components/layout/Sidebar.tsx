...
const NavItem = React.forwardRef<HTMLButtonElement, NavItemProps>(
  ({ icon, label, isActive, onClick }, ref) => (
    ...
    >
      <div
        className={`flex items-center transition-transform duration-300 ease-out group-hover:translate-x-1 ${isActive ? "translate-x-1" : ""}`}
      >
        {React.cloneElement(icon, {
          className: `w-6 h-6 mr-4 transition-colors duration-300 ${isActive ? "text-[#3A8FE0] drop-shadow-[0_0_5px_rgba(58,143,224,0.5)]" : ""}`,
        })}
...
const PlaylistSidebarItem: React.FC<PlaylistSidebarItemProps> = ({
  playlist,
  onClick,
  style,
  className,
  ...
  const imageUrl =
    playlist.coverUrl ||
    playlist.songs[0]?.image?.find((img) => img.quality === "50x50")?.url ||
    ...
    >
      {imageUrl ? (
        <img
          src={imageUrl}
          alt={playlist.name}
          className="w-10 h-10 rounded-md flex-shrink-0 object-cover opacity-80 group-hover:opacity-100 transition-opacity"
        />
      ) : (
      ...
      <div className="ml-3 min-w-0">
        <p className="font-semibold text-white truncate text-sm group-hover:text-[#3A8FE0] transition-colors">
          {playlist.name}
        </p>
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-64 glass-panel border-r border-white/5 p-4 flex-col h-full hidden md:flex z-20">
      ...
      <h3 className="px-4 text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">
        {t("library.recentPlaylists")}
      </h3>
