  ...
  ({ icon, label, isActive, onClick }, ref) => (
    ...
    >
      <div className={`flex items-center transition-transform duration-300 ease-out group-hover:translate-x-1 ${isActive ? 'translate-x-1' : ''}`}>
        {React.cloneElement(icon, { className: `w-6 h-6 mr-4 transition-colors duration-300 ${isActive ? 'text-[#3A8FE0] drop-shadow-[0_0_5px_rgba(58,143,224,0.5)]' : ''}` })}
  ...
  return (
    ...
    >
      ...
      <div className="ml-3 min-w-0">
        <p className="font-semibold text-white truncate text-sm group-hover:text-[#3A8FE0] transition-colors">{playlist.name}</p>
