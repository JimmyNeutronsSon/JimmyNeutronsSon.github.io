        ...
        <div className="flex items-center space-x-4 min-w-0 overflow-hidden relative group p-2 rounded-xl transition-colors hover:bg-white/5">
            ...
            <div className="flex-1 min-w-0 z-10 flex flex-col justify-center">
                <div className="relative overflow-hidden mask-linear-fade">
                    <h3 className="font-bold text-white truncate text-sm md:text-base leading-tight tracking-tight drop-shadow-sm cursor-default hover:text-[#3A8FE0] transition-colors" title={decodeHtml(song.name)}>
    ...
    return (
        <div className={`flex items-center gap-6 md:gap-8 ${!canControlPlayback ? 'opacity-50 pointer-events-none' : ''}`}>
            ...
            >
                ...
                {isShuffle && <span className="absolute -bottom-1 w-1 h-1 bg-[#3A8FE0] rounded-full shadow-[0_0_5px_#3A8FE0]" />}
            </button>
            ...
            >
                ...
                {repeatMode !== 'off' && <span className="absolute -bottom-1 w-1 h-1 bg-[#3A8FE0] rounded-full shadow-[0_0_5px_#3A8FE0]" />}
                {repeatMode === 'one' && <span className="absolute -top-1 -right-1 text-[8px] font-bold bg-[#3A8FE0] text-white px-1 rounded-full leading-tight">1</span>}
            </button>
        </div>
    );
};
...
const PlayerActions: React.FC<{
    ...
    return (
        <div className="flex items-center justify-end gap-1 md:gap-2">
            <button onClick={() => toggleLyrics()} title="Lyrics" className={`p-2 rounded-full transition-all duration-300 hover:bg-white/10 ${isLyricsOpen ? 'text-[#3A8FE0] bg-[#3A8FE0]/10 shadow-[0_0_10px_rgba(58,143,224,0.3)]' : 'text-gray-400 hover:text-white'}`}>
                <MicIcon className="w-5 h-5"/>
            </button>

            <button onClick={handlePartyModeClick} title={t('player.partyMode')} className={`p-2 rounded-full transition-colors hover:bg-white/10 ${partyState ? 'text-[#3A8FE0] bg-[#3A8FE0]/10' : 'text-gray-400 hover:text-white'}`}>
            ...
            <button onClick={onToggleFavorite} title={t('player.favorite')} className="p-2 rounded-full hover:bg-white/10 transition-colors group">
              <HeartIcon className={`w-5 h-5 transition-all ${isHeartAnimating ? 'scale-125' : 'group-active:scale-90'} ${userMusicContext.isFavoriteSong(song.id) ? 'fill-[#3A8FE0] text-[#3A8FE0] drop-shadow-[0_0_8px_rgba(58,143,224,0.6)]' : 'text-gray-400 group-hover:text-white'}`}/>
            </button>

            <button onClick={() => playerContext.toggleQueue()} title={t('player.showQueue')} className={`p-2 rounded-full transition-colors hover:bg-white/10 ${playerContext.isQueueOpen ? 'text-[#3A8FE0] bg-[#3A8FE0]/10' : 'text-gray-400 hover:text-white'}`}>
  ...
  return (
    <div className="relative h-full glass-panel-heavy px-4 md:px-8 grid grid-cols-[1fr_auto] md:grid-cols-[300px_1fr_300px] items-center gap-4 z-50 transition-all duration-300 shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
      ...
      <div className="absolute top-0 left-0 right-0 h-[2px] bg-white/5 md:hidden pointer-events-none">
          <div className="bg-[#3A8FE0] h-full shadow-[0_0_10px_#3A8FE0]" style={{ width: `${progress}%` }} />
      </div>
      ...
      <div className="flex md:hidden items-center gap-4 pr-2">
           <button onClick={handleToggleFavorite} title={t('player.favorite')} className="p-2">
              <HeartIcon className={`w-6 h-6 transition-all ${isHeartAnimating ? 'scale-125' : ''} ${isFav ? 'fill-[#3A8FE0] text-[#3A8FE0] drop-shadow-[0_0_5px_rgba(58,143,224,0.5)]' : 'text-gray-400'}`}/>
            </button>
            <button 
