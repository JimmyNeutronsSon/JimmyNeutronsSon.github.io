...
const SongInfo: React.FC<{
...
}> = React.memo(({ song, navigateToArtist }) => {
  const smallImage =
  ...
  return (
    <div className="flex items-center space-x-4 min-w-0 overflow-hidden relative group p-2 rounded-xl transition-colors hover:bg-white/5">
      ...
      <div
        className="absolute left-3 w-10 h-10 md:w-14 md:h-14 blur-xl opacity-60 rounded-full transition-opacity duration-700 animate-pulse"
        style={{
      ...
      <div className="flex-1 min-w-0 z-10 flex flex-col justify-center">
        <div className="relative overflow-hidden mask-linear-fade">
          <h3
            className="font-bold text-white truncate text-sm md:text-base leading-tight tracking-tight drop-shadow-sm cursor-default hover:text-[#3A8FE0] transition-colors"
...
const PlayerControls: React.FC<{
  ...
  const canControlPlayback =
    !partyState || isHost || partyState.mode === "collaborative";
  const canControlSettings = !partyState || isHost;

  const handlePlayPrev = () => {
    if (canControlPlayback) partyState ? playPrevParty() : playPrev();
  };
  const handlePlayNext = () => {
    if (canControlPlayback) partyState ? playNextParty() : playNext();
  };
  const handleTogglePlay = () => {
    if (canControlPlayback) partyState ? togglePartyPlayer() : togglePlay();
  };

  return (
    <div
      className={`flex items-center gap-6 md:gap-8 ${!canControlPlayback ? "opacity-50 pointer-events-none" : ""}`}
    >
      <button
        disabled={!canControlSettings}
        onClick={toggleShuffle}
        title={t("player.shuffle")}
        className={`group relative flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 ${isShuffle ? "text-[#3A8FE0]" : "text-gray-500 hover:text-white"}`}
      >
        <ShuffleIcon className="w-4 h-4" />
        {isShuffle && (
          <span className="absolute -bottom-1 w-1 h-1 bg-[#3A8FE0] rounded-full shadow-[0_0_5px_#3A8FE0]" />
        )}
      </button>
      ...
      <div className="flex items-center gap-4">
        ...
        >
          {isPlaying ? (
            <PauseIcon className="w-6 h-6 md:w-7 md:h-7 fill-black group-hover:fill-[#3A8FE0] transition-colors" />
          ) : (
            <PlayIcon className="w-6 h-6 md:w-7 md:h-7 fill-black group-hover:fill-[#3A8FE0] ml-1 transition-colors" />
          )}
        </button>
      ...
      <button
        ...
        className={`group relative flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 ${repeatMode !== "off" ? "text-[#3A8FE0]" : "text-gray-500 hover:text-white"}`}
      >
        ...
        {repeatMode === "one" && (
          <span className="absolute -top-1 -right-1 text-[8px] font-bold bg-[#3A8FE0] text-white px-1 rounded-full leading-tight">
            1
          </span>
        )}
      </button>
...
const PlayerProgressBar: React.FC<{
  ...
  return (
    <div className="w-full flex items-center space-x-3 mt-1 group/bar select-none">
      <span className="text-xs font-medium text-gray-500 w-10 text-right tabular-nums tracking-wide">
      ...
      >
        <div className="w-full h-1 bg-white/10 rounded-full relative overflow-visible group-hover/bar:h-1.5 transition-all duration-300">
          <div
            className="absolute h-full bg-white rounded-full group-hover/bar:bg-[#3A8FE0] shadow-[0_0_10px_rgba(255,255,255,0.2)] group-hover/bar:shadow-[0_0_15px_rgba(58,143,224,0.5)] transition-all duration-100"
      ...
      <span className="text-xs font-medium text-gray-500 w-10 text-left tabular-nums tracking-wide">
        {formatTime(duration)}
      </span>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-end gap-1 md:gap-2">
      <button
        ...
        title="Lyrics"
        className={`p-2 rounded-full transition-all duration-300 hover:bg-white/10 ${isLyricsOpen ? "text-[#3A8FE0] bg-[#3A8FE0]/10 shadow-[0_0_10px_rgba(58,143,224,0.3)]" : "text-gray-400 hover:text-white"}`}
      >
        <MicIcon className="w-5 h-5" />
      </button>

      <button
        onClick={handlePartyModeClick}
        title={t("player.partyMode")}
        className={`p-2 rounded-full transition-colors hover:bg-white/10 ${partyState ? "text-[#3A8FE0] bg-[#3A8FE0]/10" : "text-gray-400 hover:text-white"}`}
      >
      ...
      <button
        ...
        className="p-2 rounded-full text-gray-400 hover:text-white hover:bg-white/10 transition-colors disabled:opacity-50"
      >
        {isDownloading ? (
      ...
      <button
        ...
        className={`p-2 rounded-full transition-colors hover:bg-white/10 ${openModal ? "text-white" : "text-gray-400 hover:text-white"}`}
      >
        <PlusCircleIcon
          className={`w-5 h-5 transition-transform duration-300 ${openModal ? "rotate-[135deg]" : ""}`}
        />
      </button>
      ...
      <SmartMenu
        ...
        triggerRef={playlistButtonRef}
      >
        <div className="flex flex-col py-1">
          <p className="px-3 py-1.5 text-xs text-gray-400 font-bold uppercase">
          ...
          <button
            ...
            className="w-full text-left px-3 py-1.5 text-sm rounded-md hover:bg-white/10 text-white"
          >
            {t("player.newPlaylist")}
          </button>
          <hr className="my-1 border-white/10" />
          <div className="max-h-48 overflow-y-auto custom-scrollbar">
            {userMusicContext.playlists.length > 0 ? (
              userMusicContext.playlists.map((p) => (
                <button
                  key={p.id}
                  onClick={() => {
                    userMusicContext.addSongToPlaylist(p.id, song);
                    setOpenModal(false);
                  }}
                  className="w-full text-left px-3 py-1.5 text-sm rounded-md hover:bg-white/10 truncate text-white"
                >
                  {p.name}
                </button>
              ))
      ...
      >
        <HeartIcon
          className={`w-5 h-5 transition-all ${isHeartAnimating ? "scale-125" : "group-active:scale-90"} ${userMusicContext.isFavoriteSong(song.id) ? "fill-[#3A8FE0] text-[#3A8FE0] drop-shadow-[0_0_8px_rgba(58,143,224,0.6)]" : "text-gray-400 group-hover:text-white"}`}
        />
      </button>

      <button
        onClick={() => playerContext.toggleQueue()}
        title={t("player.showQueue")}
        className={`p-2 rounded-full transition-colors hover:bg-white/10 ${playerContext.isQueueOpen ? "text-[#3A8FE0] bg-[#3A8FE0]/10" : "text-gray-400 hover:text-white"}`}
      >
      ...
      <div className="flex items-center group/volume ml-2 pl-2 border-l border-white/10 h-8">
        ...
        <div className="w-0 group-hover/volume:w-24 transition-all duration-300 overflow-hidden flex items-center mx-1">
          <input
            ...
            value={volume}
            onChange={(e) =>
              playerContext.setVolume(parseFloat(e.target.value))
            }
...
export const Player: React.FC<PlayerProps> = ({ navigateToArtist }) => {
  ...
  const displayedSong =
    isHost || !partyState ? playerContext.currentSong : partyState.currentSong;
  const isPlaying =
    isHost || !partyState ? playerContext.isPlaying : partyState.isPlaying;
  const currentTime =
    isHost || !partyState ? playerContext.currentTime : partyState.currentTime;
  const duration = displayedSong?.duration ?? 0;

  const handleDownload = async () => {
    if (!displayedSong || isDownloading) return;
    const songUrl = displayedSong.downloadUrl.find(
      (q) => q.quality === playerContext.currentQuality,
    )?.url;
    if (!songUrl) return;
  ...
  const handleToggleFavorite = () => {
    ...
    const isCurrentlyFavorite = userMusicContext.isFavoriteSong(
      displayedSong.id,
    );
  ...
  return (
    <div className="relative h-full glass-panel-heavy px-4 md:px-8 grid grid-cols-[1fr_auto] md:grid-cols-[300px_1fr_300px] items-center gap-4 z-50 transition-all duration-300 shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
      ...
      <div className="flex md:hidden items-center gap-4 pr-2">
        ...
        >
          <HeartIcon
            className={`w-6 h-6 transition-all ${isHeartAnimating ? "scale-125" : ""} ${isFav ? "fill-[#3A8FE0] text-[#3A8FE0] drop-shadow-[0_0_5px_rgba(58,143,224,0.5)]" : "text-gray-400"}`}
          />
        </button>
        <button
          onClick={handleMobileTogglePlay}
          className={`w-12 h-12 bg-white rounded-full flex items-center justify-center text-black shadow-lg shadow-white/10 active:scale-95 border-2 border-transparent ${isPlaying ? "" : "border-[#3A8FE0]"}`}
        >
          {isPlaying ? (
