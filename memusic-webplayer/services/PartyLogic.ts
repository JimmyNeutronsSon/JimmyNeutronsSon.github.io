...
const PEER_ID_PREFIX = "welkin-music-party-v3-";
...
export class PartyHost {
      ...
      try {
        ...
        peer.on("open", () => {
          ...
          const initialState: PartyState = {
            ...
            currentQueue: playerState.currentQueue.map((s: any) => ({
              ...s,
              addedBy: this.myId,
            })),
  ...
  private handleDataPacket(conn: any, data: any) {
    ...
    switch (data.type) {
      case "JOIN":
        if (
          !newState.participants.some(
            (p: PartyParticipant) => p.id === data.payload.id,
          )
        ) {
        ...
        try {
          conn.send({ type: "JOIN_ACK", payload: { success: true } });
        } catch (e) {}
        break;

      case "LEAVE":
        newState.participants = newState.participants.filter(
          (p: PartyParticipant) => p.id !== data.senderId,
        );
        changed = true;
        break;
      ...
      case "ADD_SONG":
        if (newState.mode === "collaborative") {
          ...
          const lastSong =
            newState.currentQueue[newState.currentQueue.length - 1];
      ...
      case "REMOVE_SONG":
        if (newState.mode === "collaborative") {
          const songIndex = newState.currentQueue.findIndex(
            (s: PartyQueueSong) =>
              s.id === data.payload && s.addedBy === data.senderId,
          );
          if (songIndex !== -1) {
