...
export const synchronizePlayback = (
  ...
  if (currentSong?.id !== player.currentSong?.id) {
    if (currentSong) {
      ...
      player.playSong(currentSong, currentQueue, {
        type: "party",
        id: partyId,
      });
  ...
  const secondsPassedSinceUpdate = Math.max(
    0,
    (now - localStateUpdateTime) / 1000,
  );
  const estimatedHostTime = isPlaying
    ? currentTime + secondsPassedSinceUpdate
    : currentTime;
  ...
  if (
    isPlaying &&
    player.isPlaying &&
    currentSong?.id === player.currentSong?.id
  ) {
