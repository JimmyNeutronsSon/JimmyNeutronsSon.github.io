...
const STORAGE_KEY = "welkin-music-v1-storage";
...
const strictMerge = (defaults: any, source: any): any => {
  ...
  if (Array.isArray(defaults)) {
    return Array.isArray(source)
      ? source.filter((i: any) => i !== null && i !== undefined)
      : defaults;
  }
    ...
    Object.keys(defaults).forEach((key) => {
      output[key] =
        source[key] === undefined
          ? defaults[key]
          : strictMerge(defaults[key], source[key]);
    });
    return output;
  }
...
const validateSpecificLogic = (state: AppState): AppState => {
  if (state.settings.player.eqSettings.length !== 5)
    state.settings.player.eqSettings =
      defaultAppState.settings.player.eqSettings;
...
const loadState = (): AppState => {
  try {
    ...
    return validateSpecificLogic(
      strictMerge(defaultAppState, JSON.parse(serializedState)),
    );
  } catch (error) {
  ...
  try {
    ...
    if (error.name === "QuotaExceededError" || error.code === 22) {
      ...
      if (state.music.history.length > 10) {
        state.music.history = state.music.history.slice(
          0,
          Math.floor(state.music.history.length / 2),
        );
        saveState(state);
        return;
      }
...
export const useStorage = (): [
  AppState,
  (updater: (draft: AppState) => void) => void,
] => {
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === STORAGE_KEY && e.newValue) {
        try {
          setAppState(
            validateSpecificLogic(
              strictMerge(defaultAppState, JSON.parse(e.newValue)),
            ),
          );
        } catch (error) {
  ...
  const updateState = useCallback((updater: (draft: AppState) => void) => {
    setAppState((currentState) => {
      try {
        ...
        for (const key of Object.keys(defaultAppState) as Array<
          keyof AppState
        >) {
