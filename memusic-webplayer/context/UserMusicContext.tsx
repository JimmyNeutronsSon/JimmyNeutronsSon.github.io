...
export const UserMusicContext = createContext<UserMusicContextType>(
  {} as UserMusicContextType,
);
...
export const UserMusicProvider: React.FC<UserMusicProviderProps> = ({
  children,
  musicData,
  fullState,
  setAppState,
  ...
  const toggleFavoriteAlbum = (album: Album) => {
    updateMusicData((draft) => {
      ...
      if (isFav)
        draft.favoriteAlbums = draft.favoriteAlbums.filter(
          (a) => a.id !== album.id,
        );
  ...
  const isFavoriteApiPlaylist = useCallback(
    (playlistId: string) =>
      favoriteApiPlaylists.some((p) => p.id === playlistId),
    [favoriteApiPlaylists],
  );
  const toggleFavoriteApiPlaylist = (playlist: Playlist) => {
    updateMusicData((draft) => {
      const isFav = draft.favoriteApiPlaylists.some(
        (p) => p.id === playlist.id,
      );
      if (isFav)
        draft.favoriteApiPlaylists = draft.favoriteApiPlaylists.filter(
          (p) => p.id !== playlist.id,
        );
  ...
  const toggleFavoriteArtist = (artist: Artist | FullArtist) => {
    ...
    updateMusicData((draft) => {
      ...
      if (isFav)
        draft.favoriteArtists = draft.favoriteArtists.filter(
          (a) => a.id !== artist.id,
        );
  ...
  const createPlaylist = (
    ...
    updateMusicData((draft) => {
      draft.playlists.unshift(newPlaylist);
    });
    return newPlaylist;
  };
  ...
  const deletePlaylist = (playlistId: string) => {
    updateMusicData((draft) => {
      ...
      draft.playlistHistory = draft.playlistHistory.filter(
        (id) => id !== playlistId,
      );
    });
  };
  ...
  const updatePlaylist = (
    ...
    updateMusicData((draft) => {
      ...
      if (index > -1)
        draft.playlists[index] = { ...draft.playlists[index], ...updates };
    });
  };
  ...
  const addToHistory = (song: Song) => {
    updateMusicData((draft) => {
      draft.history = [
        song,
        ...draft.history.filter((s) => s.id !== song.id),
      ].slice(0, 50);
    });
  };
  ...
  const addToPlaylistHistory = (playlistId: string) => {
    updateMusicData((draft) => {
      draft.playlistHistory = [
        playlistId,
        ...draft.playlistHistory.filter((id) => id !== playlistId),
      ].slice(0, 10);
    });
  };
  ...
  const importData = (
    ...
    try {
      ...
      if (typeof data !== "object" || data === null) {
        return {
          success: false,
          messageKey: "settings.data.importInvalidFormat",
        };
      }

      setAppState((draft) => {
        if (data.profile)
          draft.profile = { ...defaultAppState.profile, ...data.profile };
        if (data.settings)
          draft.settings = { ...defaultAppState.settings, ...data.settings };
        ...
        if (mode === "replace") {
          ...
          draft.music.favoriteSongs = mergeUniqueById(
            draft.music.favoriteSongs,
            musicData.favoriteSongs,
          );
          draft.music.favoriteAlbums = mergeUniqueById(
            draft.music.favoriteAlbums,
            musicData.favoriteAlbums,
          );
          draft.music.playlists = mergeUniqueById(
            draft.music.playlists,
            musicData.playlists,
          );
          draft.music.favoriteApiPlaylists = mergeUniqueById(
            draft.music.favoriteApiPlaylists,
            musicData.favoriteApiPlaylists,
          );
          draft.music.favoriteArtists = mergeUniqueById(
            draft.music.favoriteArtists,
            musicData.favoriteArtists,
          );

          if (Array.isArray(musicData.history)) {
            const combined = [...musicData.history, ...draft.music.history];
            draft.music.history = combined
              .filter(
                (song, index, self) =>
                  song &&
                  song.id &&
          ...
          if (Array.isArray(musicData.playlistHistory)) {
            draft.music.playlistHistory = [
              ...new Set([
                ...musicData.playlistHistory,
                ...musicData.playlistHistory,
              ]),
            ].slice(0, 10);
          }
