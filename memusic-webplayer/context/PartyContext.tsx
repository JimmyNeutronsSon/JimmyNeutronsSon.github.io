...
export const PartyProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [myId] = useLocalStorage<string>("welkin-music-party-userId", uuidv4());
  ...
  const hostCallbacks = {
    ...
    onAddSongs: (songs: Song[]) =>
      playerContextRef.current.addSongsToEnd(songs),
    onRemoveSong: (id: string) =>
      playerContextRef.current.removeSongFromQueue(id),
  };

  const guestCallbacks = {
    onStateUpdate: (newState: PartyState | null) => {
      if (newState === null) {
        setPartyEndedMessage({
          key: "modals.partyEnded.endedByHost",
          replacements: { partyId: partyStateRef.current?.partyId || "" },
        });
        leaveParty();
      } else {
  ...
  useEffect(() => {
    ...
    const syncInterval = setInterval(() => {
      const { currentSong, isPlaying, currentTime, currentQueue } =
      ...
      setPartyState((prev) => {
        ...
        const queueChanged =
          prev.currentQueue.length !== currentQueue.length ||
          (prev.currentQueue.length > 0 &&
            currentQueue.length > 0 &&
            prev.currentQueue[0].id !== currentQueue[0].id);
        ...
        const newState: PartyState = {
          ...
          isPlaying,
          currentTime,
          currentQueue: currentQueue.map((s) => ({
            ...s,
            addedBy: (s as PartyQueueSong).addedBy || myId,
          })),
          hostPing: Date.now(),
        };
        ...
        if (
          ...
          playingChanged ||
          queueChanged ||
          Date.now() - prev.lastStateUpdate > 1500
        ) {
  ...
  useEffect(() => {
    ...
    const syncInterval = setInterval(() => {
      synchronizePlayback(
        partyState,
        playerContextRef.current,
        guestRef.current?.timeOffset || null,
      );
    }, 500);
  ...
  const addSongToPartyQueue = (song: Song) => {
    if (isHost) {
      ...
      setPartyState((prev) => {
        ...
        const newState = {
          ...prev,
          currentQueue: [...prev.currentQueue, songToAdd],
        };
  ...
  const removeSongFromPartyQueue = (songId: string) => {
    if (isHost) {
      ...
      setPartyState((prev) => {
        ...
        const newState = {
          ...prev,
          currentQueue: prev.currentQueue.filter((s) => s.id !== songId),
        };
        hostRef.current?.broadcast(newState);
        return newState;
      });
  ...
  return (
    <PartyContext.Provider
      value={{
        ...
        playPrevParty,
        addSongToPartyQueue,
        removeSongFromPartyQueue,
        reorderPartyQueue,
        sendReaction,
        partyEndedMessage,
        clearPartyEndedMessage,
      }}
    >
      {children}
    </PartyContext.Provider>
