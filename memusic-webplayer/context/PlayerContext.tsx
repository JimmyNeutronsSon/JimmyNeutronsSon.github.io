...
const createImpulseResponse = (context: AudioContext): AudioBuffer => {
  ...
  for (let channel = 0; channel < numChannels; channel++) {
    ...
    for (let i = 0; i < frameCount; i++) {
      channelData[i] =
        (Math.random() * 2 - 1) *
        Math.pow(1 - i / sampleRate / duration, decay);
    }
  }
  return buffer;
};
...
const smartShuffle = (songs: Song[]): Song[] => {
  ...
  for (let i = 1; i < shuffled.length; i++) {
    if (
      shuffled[i].artists.primary[0]?.id ===
      shuffled[i - 1].artists.primary[0]?.id
    ) {
      const swapIndex = shuffled.findIndex(
        (song, j) =>
          j > i &&
          song.artists.primary[0]?.id !== shuffled[i].artists.primary[0]?.id,
      );
      if (swapIndex !== -1)
...
export const PlayerProvider: React.FC<PlayerProviderProps> = ({
  ...
  const [autoplayStartIndex, setAutoplayStartIndex] = useState<number | null>(
    null,
  );
  ...
  const updatePlayerSettings = (updater: (draft: PlayerSettings) => void) =>
    setAppState((s) => {
      updater(s.settings.player);
    });
  const updatePlayerQueue = (updater: (draft: PlayerQueueState) => void) =>
    setAppState((s) => {
      updater(s.playerQueue);
    });
  ...
  useEffect(() => {
    ...
    const initAudioContext = () => {
      ...
      pannerGain.connect(panner.positionZ);
      pannerGain.connect(pannerDelay);
      pannerDelay.connect(panner.positionX);
    ...
    updatePlayerQueue((d) => {
      d.currentIndex =
        d.currentQueue.length === 0
          ? -1
          : (d.currentIndex + 1) % d.currentQueue.length;
    });
  }, [updatePlayerQueue]);
  ...
  useEffect(() => {
    if (
      !currentSong ||
      (currentSong.downloadUrl && currentSong.downloadUrl.length > 0)
    )
      return;
    let isCancelled = false;
    getSongsByIds([currentSong.id])
      .then((res) => {
        if (isCancelled) return;
        const fullSongData =
          res.success && res.data.length > 0 ? res.data[0] : null;
        if (fullSongData && fullSongData.downloadUrl?.length > 0) {
          updatePlayerQueue((d) => {
            d.currentQueue = d.currentQueue.map((s) =>
              s.id === currentSong.id ? fullSongData : s,
            );
          });
        } else {
          playNext();
  ...
  const fetchRecommendations = useCallback(
    async (seedSong: Song): Promise<Song[]> => {
      const collected = new Map<string, Song>([[seedSong.id, seedSong]]);
      const addUnique = (list: Song[]) =>
        list.forEach((s) => {
          if (!collected.has(s.id)) collected.set(s.id, s);
        });
  ...
  const populateQueueWithRadio = useCallback(
    async (seedSong: Song) => {
      try {
        ...
        updatePlayerQueue((d) => {
          ...
          if (songsToAdd.length > 0) {
            ...
            setAutoplayStartIndex((prev) =>
              prev === null ? oldQueueSize : prev,
            );
  ...
  useEffect(() => {
    ...
    if (currentSong) {
      if (
        currentSong.id !== audio.dataset.songId ||
        selectedQuality !== audio.dataset.quality
      ) {
        const getUrl = (q: string) =>
          currentSong.downloadUrl?.find((i) => i.quality === q)?.url;
        const songUrl =
          getUrl(selectedQuality) ||
          getUrl("320kbps") ||
          getUrl("160kbps") ||
        ...
        if (songUrl) {
          ...
          const newQuality =
            currentSong.downloadUrl.find(
              (q) => q.url.replace(/^http:/, "https:") === httpsUrl,
            )?.quality || null;
          const timeToSeek =
            audio.dataset.songId === currentSong.id ? audio.currentTime : 0;
  ...
  const toggleShuffle = () => {
    ...
    updatePlayerSettings((d) => {
      d.isShuffle = newShuffleState;
    });
    updatePlayerQueue((d) => {
      if (newShuffleState) {
        if (d.currentQueue.length > 1) {
          ...
          const others = d.currentQueue.filter(
            (s) => s.id !== currentlyPlaying.id,
          );
  ...
  const toggleEq = () =>
    updatePlayerSettings((d) => {
      d.isEqEnabled = !d.isEqEnabled;
    });
  const toggle8D = () =>
    updatePlayerSettings((d) => {
      d.is8DEnabled = !d.is8DEnabled;
    });
  const toggleReverb = () =>
    updatePlayerSettings((d) => {
      d.isReverbEnabled = !d.isReverbEnabled;
    });

  const setEqGain = (bandIndex: number, gain: number) =>
    updatePlayerSettings((d) => {
      d.eqSettings[bandIndex].gain = gain;
    });
  const resetEq = () =>
    updatePlayerSettings((d) => {
      d.eqSettings = eqBands.map(() => ({ gain: 0 }));
    });
  ...
  useEffect(() => {
    ...
    pannerGain.gain.setTargetAtTime(
      is8DEnabled && isPlaying ? 2.5 : 0,
      context.currentTime,
      0.5,
    );
  ...
  const playSong = useCallback(
    async (
      ...
      updatePlayerQueue((d) => {
        ...
        if (
          playContext.id === d.contextId &&
          playContext.type === d.contextType &&
          playContext.type !== "search"
        ) {
  ...
  const playRadio = useCallback(
    async (song: Song) => {
      ...
      const radioQueue = [song, ...suggestions].filter(
      ...
      updatePlayerQueue((d) => {
        if (isShuffle) {
          ...
          d.currentQueue = [
            radioQueue[0],
            ...smartShuffle(radioQueue.slice(1)),
          ];
        } else {
          d.originalQueueUnshuffled = null;
  ...
  const addSongsToEnd = (songs: Song[]) => {
    updatePlayerQueue((d) => {
      const songsToAdd = songs.filter(
        (s) => !d.currentQueue.some((qs) => qs.id === s.id),
      );
      d.currentQueue.push(...songsToAdd);
  ...
  const reorderQueue = (oldIndex: number, newIndex: number) => {
    ...
    updatePlayerQueue((d) => {
      ...
      d.currentIndex = d.currentQueue.findIndex(
        (s) => s.id === d.currentQueue[d.currentIndex]?.id,
      );
    });
  };
  ...
  const removeSongFromQueue = (songId: string) => {
    updatePlayerQueue((d) => {
      ...
      if (idx === -1) return;
      if (autoplayStartIndex !== null && idx < autoplayStartIndex)
        setAutoplayStartIndex((p) => (p !== null ? p - 1 : null));
      ...
      if (idx < d.currentIndex) d.currentIndex--;
        ...
        wasPlaying &&
        d.currentQueue.length > 0 &&
        d.currentQueue.length <= d.currentIndex
      )
        d.currentIndex = 0;
      ...
      if (d.currentQueue.length === 0) {
        ...
        d.contextType = null;
        setAutoplayStartIndex(null);
      } else if (wasPlaying && audioRef.current)
  ...
  const moveSongInQueue = (songId: string, direction: "top" | "bottom") => {
    ...
    updatePlayerQueue((d) => {
      ...
      d.currentQueue.splice(
        direction === "top" ? d.currentIndex + 1 : d.currentQueue.length,
        0,
        song,
      );
    });
    ...
    if ("mediaSession" in navigator) {
      if (currentSong) {
        ...
        navigator.mediaSession.setActionHandler("seekbackward", (d) =>
          seek(Math.max(currentTime - (d.seekOffset || 10), 0)),
        );
        navigator.mediaSession.setActionHandler("seekforward", (d) =>
          seek(Math.min(currentTime + (d.seekOffset || 10), duration)),
        );
      } else {
        ...
        [
          ...
          navigator.mediaSession.setActionHandler(h as any, null),
        );
      }
    }
  }, [
  ...
  const contextValue: PlayerContextType = {
    ...
    setReverbMix,
    setPlaybackRate,
    setIsShuffle: (s: boolean) =>
      updatePlayerSettings((d) => {
        d.isShuffle = s;
      }),
    setRepeatMode,
    setIsEqEnabled: (e: boolean) =>
      updatePlayerSettings((d) => {
        d.isEqEnabled = e;
      }),
    setIs8DEnabled: (e: boolean) =>
      updatePlayerSettings((d) => {
        d.is8DEnabled = e;
      }),
    setIsReverbEnabled: (e: boolean) =>
      updatePlayerSettings((d) => {
        d.isReverbEnabled = e;
      }),
